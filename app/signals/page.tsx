...
async function loadSignalRows(email: string, owner: boolean) {
    ...
    <article style={{ ...styles.card, borderColor: `${tone}66` }}>
      ...
      <div style={styles.body}>
        <p style={{ ...styles.eyebrow, color: tone }}>
          {priorityOf(row)} · score {scoreOf(row)}
        </p>
...
export default function SignalsPage() {
  ...
  async function load() {
    ...
    try {
      const loaded = await loadSignalRows(currentEmail, currentOwner);
      setRows(loaded);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) =>
      [titleOf(row), noteOf(row), marketOf(row), priorityOf(row), signalIdOf(row), itemIdOf(row), ownerEmailOf(row)]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
      ...
      <div style={styles.wrap}>
        ...
        <section style={styles.hero}>
          ...
          <p style={{ ...styles.muted, maxWidth: 820, fontSize: 18, marginTop: 16 }}>
            Signals use window cards with photos and Message Owner routes to the working simple Connect system.
          </p>

          <div style={{ marginTop: 16 }}>
            <span style={styles.chip}>Signed in: {email || "unknown"}</span>
            <span style={styles.chip}>Signals: {rows.length}</span>
        ...
        <section style={styles.hero}>
          <p style={styles.eyebrow}>Search Signals</p>
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title, market, owner, signal id..." style={styles.input} />
        </section>
        ...
        <section style={styles.grid}>
          {filtered.map((row, index) => (
            <WindowCard key={`${signalIdOf(row)}-${itemIdOf(row)}-${index}`} row={row} email={email} />
          ))}
        </section>
      </div>
    </main>
