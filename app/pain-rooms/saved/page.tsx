  ...
  return (
    <main style={{minHeight:"100vh",background:"#05070d",color:"#fff",padding:16}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        {pains.length === 0? <div style={{opacity:0.7}}>No saved pain points.</div> : (
            ...
            {pains.map((p:any) => (
              <div key={p.id} style={{border:"1px solid #222",borderRadius:12,padding:16}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>handleUnsave(p.id)} style={{padding:"6px 12px",borderRadius:6,background:"#ff4444",color:"#fff",fontSize:12}}>Unsave</button>
                  <button onClick={()=>handleMessage(p.postedBy)} style={{padding:"6px 12px",borderRadius:6,background:"#FFD700",color:"#000",fontSize:12,fontWeight:900}}>Message</button>
                </div>
              </div>
            ))}
          </div>
