...
export default function Page(){
 ...
 async function submit(){
   setMsg("Compressing photo and saving deal room...");
   const urls=await photoDataUrls(files,10);
   const attempted=files?files.length:0;
   const id=saveRoom("deal",{...f,...photoFields(urls,attempted),title:txt(f.title,"Untitled Deal Room"),analyzer:`Deal analyzer: ${asset} ${txt(f.propertyType)} in ${txt(f.city)}, ${txt(f.county)}, ${txt(f.state)}. Condition ${txt(f.condition)}. Occupancy ${txt(f.occupancy)}. Spread estimate ${fmt(spread)}. Route to ${list(f.routeTo).join(", ")||"matched members"}. Timeline ${txt(f.timeline)}.`});
 ...
 <section style={hero}><div style={eyebrow}>Adaptive Deal Intake</div><h1 style={h1}>Deal Opportunity.</h1><p style={sub}>Residential, Commercial, and Land switch fields correctly. Photos save as compressed room cover/gallery.</p></section>

 <Section title="Asset + Strategy"><SingleChips labelName="Asset Type" options={ASSET} value={asset} onChange={setAsset}/><div style={{height:18}}/><MultiChips labelName="Strategy" options={STRATEGIES} selected={list(f.strategy)} onToggle={v=>tog("strategy",v)}/><div style={{height:18}}/><MultiChips labelName="Route To" options={ROUTE} selected={list(f.routeTo)} onToggle={v=>tog("routeTo",v)}/></Section>
 <Section title={`${asset} Property + Market`}><div style={grid}><Field l="Deal Title" v={txt(f.title)} c={v=>up("title",v)}/><SelectBox l="State" v={txt(f.state,"GA")} c={v=>up("state",v)} options={STATES}/><Field l="City" v={txt(f.city)} c={v=>up("city",v)}/><Field l="County" v={txt(f.county)} c={v=>up("county",v)}/><Field l="Address / Location" v={txt(f.address)} c={v=>up("address",v)}/><SelectBox l={`${asset} Type`} v={txt(f.propertyType,typeOptions[0])} c={v=>up("propertyType",v)} options={typeOptions}/></div></Section>
 <Section title="Deal Intelligence Selectors"><div style={grid}>{asset!=="Land"?<SelectBox l="Condition" v={txt(f.condition,"Medium Rehab")} c={v=>up("condition",v)} options={CONDITION}/>:<SelectBox l="Land Status" v={txt(f.condition,"Raw Land")} c={v=>up("condition",v)} options={["Raw Land","Cleared","Wooded","Entitled","Needs Rezoning","Approved Plans","Tear Down"]}/>} {asset!=="Land"?<SelectBox l="Occupancy" v={txt(f.occupancy,"Vacant")} c={v=>up("occupancy",v)} options={OCCUPANCY}/>:<SelectBox l="Access" v={txt(f.access,"Unknown")} c={v=>up("access",v)} options={["Unknown","Road Frontage","Easement","Landlocked","Private Road","No Access"]}/>}<SelectBox l="Deal Strength" v={txt(f.dealStrength,"Moderate")} c={v=>up("dealStrength",v)} options={STRENGTH}/><SelectBox l="Timeline" v={txt(f.timeline,"14 Days")} c={v=>up("timeline",v)} options={TIMELINE}/><SelectBox l="Title Status" v={txt(f.titleStatus,"Unknown")} c={v=>up("titleStatus",v)} options={["Unknown","Clear","Clouded","Probate","Lien Issue","Needs Attorney"]}/><SelectBox l="Flood Zone" v={txt(f.floodZone,"Unknown")} c={v=>up("floodZone",v)} options={YESNO}/></div><div style={{height:18}}/><MultiChips labelName="Seller Motivation" options={MOTIVATION} selected={list(f.motivation)} onToggle={v=>tog("motivation",v)}/></Section>
 <Section title="Numbers"><div style={grid}><Field l="Ask Price" v={txt(f.askingPrice)} c={v=>up("askingPrice",v)}/><Field l="Property Value / ARV" v={txt(f.propertyValue)} c={v=>up("propertyValue",v)}/>{asset!=="Land"?<Field l="Repairs / Work" v={txt(f.repairs)} c={v=>up("repairs",v)}/>:<Field l="Site Work / Development Cost" v={txt(f.repairs)} c={v=>up("repairs",v)}/>}<Field l="Exit Value" v={txt(f.exitValue)} c={v=>up("exitValue",v)}/><Field l="Assignment Fee" v={txt(f.assignmentFee)} c={v=>up("assignmentFee",v)}/>{asset!=="Land"?<Field l="Monthly Rent" v={txt(f.monthlyRent)} c={v=>up("monthlyRent",v)}/>:<Field l="Projected Lot / Pad Value" v={txt(f.monthlyRent)} c={v=>up("monthlyRent",v)}/>}<Field l="Taxes" v={txt(f.taxes)} c={v=>up("taxes",v)}/><Field l="Insurance" v={txt(f.insurance)} c={v=>up("insurance",v)}/>{asset==="Commercial"?<Field l="NOI" v={txt(f.noi)} c={v=>up("noi",v)}/>:null}{asset==="Commercial"?<Field l="Cap Rate" v={txt(f.capRate)} c={v=>up("capRate",v)}/>:null}</div><div style={{...grid,marginTop:16}}><div style={calcBox}>Equity Spread: {fmt(spread)}</div><div style={calcBox}>Estimated Profit: {fmt(profit)}</div></div></Section>
 <Section title={`${asset} Facts`}><div style={grid}>{asset==="Residential"?<><Field l="Beds" v={txt(f.beds)} c={v=>up("beds",v)}/><Field l="Baths" v={txt(f.baths)} c={v=>up("baths",v)}/><Field l="Sqft" v={txt(f.sqft)} c={v=>up("sqft",v)}/><Field l="Units" v={txt(f.units)} c={v=>up("units",v)}/></>:null}{asset==="Commercial"?<><Field l="Building Size" v={txt(f.buildingSize)} c={v=>up("buildingSize",v)}/><Field l="Units / Suites" v={txt(f.units)} c={v=>up("units",v)}/><Field l="NOI" v={txt(f.noi)} c={v=>up("noi",v)}/><Field l="Cap Rate" v={txt(f.capRate)} c={v=>up("capRate",v)}/></>:null}{asset==="Land"?<><Field l="Acres" v={txt(f.acres)} c={v=>up("acres",v)}/><Field l="Zoning" v={txt(f.zoning)} c={v=>up("zoning",v)}/><Field l="Frontage" v={txt(f.frontage)} c={v=>up("frontage",v)}/><SelectBox l="Utilities Available" v={txt(f.utilitiesAvailable,"Unknown")} c={v=>up("utilitiesAvailable",v)} options={["Unknown","Water","Sewer","Power","All Utilities","None","Needs Extension"]}/></>:null}</div></Section>
 <Section title="Risk + Docs"><div style={grid}><Field l="Existing Loan Balance" v={txt(f.loanBalance)} c={v=>up("loanBalance",v)}/><Field l="Interest Rate" v={txt(f.interestRate)} c={v=>up("interestRate",v)}/><SelectBox l="Seller Flexibility" v={txt(f.sellerFlexibility,"Unknown")} c={v=>up("sellerFlexibility",v)} options={["Unknown","Low","Medium","High","Creative Terms Available"]}/><SelectBox l="Environmental Issues" v={txt(f.environmentalIssues,"Unknown")} c={v=>up("environmentalIssues",v)} options={["Unknown","None Known","Possible","Confirmed","Needs Report"]}/><SelectBox l="Permits Needed" v={txt(f.permitsNeeded,"Unknown")} c={v=>up("permitsNeeded",v)} options={["Unknown","None","Minor","Major","Stop Work / City Issue"]}/><SelectBox l="City / Code Issues" v={txt(f.cityIssues,"Unknown")} c={v=>up("cityIssues",v)} options={["Unknown","None Known","Violation","Condemned","Inspection Required"]}/></div><div style={{height:18}}/><MultiChips labelName="Attached Docs Checklist" options={DOCS} selected={list(f.docs)} onToggle={v=>tog("docs",v)}/></Section>
 <Section title="Contact"><div style={grid}><Field l="Contact Name" v={txt(f.contactName)} c={v=>up("contactName",v)}/><Field l="Phone" v={txt(f.contactPhone)} c={v=>up("contactPhone",v)}/><Field l="Email" v={txt(f.contactEmail)} c={v=>up("contactEmail",v)}/><SelectBox l="Best Contact" v={txt(f.bestContact,"VaultForge Message")} c={v=>up("bestContact",v)} options={["VaultForge Message","Text","Phone","Email","Contact Form"]}/><SelectBox l="Submitter Role" v={txt(f.submitterRole,"Owner")} c={v=>up("submitterRole",v)} options={["Owner","Wholesaler","Agent","Investor","Lender","Operator","Partner"]}/></div></Section>
