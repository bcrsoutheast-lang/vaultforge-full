...
type DealRoom = {
  ...
  memberRoomStatus: RoomState | "sold" | "resolved";
...
const LEGACY_STORE_KEY = "vaultforge_clean_deal_rooms";
const CANONICAL_STORE_KEY = "vaultforge_canonical_deal_rooms_v1";
...
function currentOwner() {
  if (!browserReady()) {
    return { ownerId: "local_member", ownerEmail: "", ownerName: "VaultForge Member", createdBy: "local_member", createdByEmail: "" };
  }
  ...
  for (const key of profileKeys) {
    ...
    if (profile && typeof profile === "object") {
      ...
      const name = safeText(profile.name || profile.fullName || profile.full_name || profile.company || "VaultForge Member");
      const id = safeText(profile.id || email || "local_member");
      return { ownerId: id, ownerEmail: email, ownerName: name, createdBy: id, createdByEmail: email };
    }
  }

  const fallbackEmail =
    safeText(window.localStorage.getItem("vf_email")) ||
    safeText(window.localStorage.getItem("vaultforge_email")) ||
    safeText(window.localStorage.getItem("member_email")) ||
    safeText(window.localStorage.getItem("email"));

  return {
    ownerId: fallbackEmail || "local_member",
    ownerEmail: fallbackEmail,
    ownerName: "VaultForge Member",
...
function defaultDeal(): DealRoom {
  ...
  return {
    ...
    capRate: "",
    monthlyRent: "",
    leasesInPlace: "Unknown",
    tenantQuality: "Unknown",
    acres: "",
    zoning: "",
    roadFrontage: "",
    utilities: "Unknown",
    entitlementStatus: "Unknown",
    surveyAvailable: "Unknown",
    soilTest: "Unknown",
    topoAvailable: "Unknown",
    accessRoad: "Unknown",
    contactName: "",
    contactPhone: "",
    contactEmail: "",
...
function normalizeDeal(row: any): DealRoom {
  const base = defaultDeal();
  const id = safeText(row?.id || row?.roomId);
  const photos = safeList(row?.photos || row?.photoUrls);
  const cover = safeText(row?.coverPhoto || row?.photoUrl || row?.imageUrl || photos[0]);
  ...
  return {
    ...
    photoUrl: cover,
    imageUrl: cover,
    status: row?.status || row?.roomState || row?.cleanupState || row?.stateStatus || "active",
    roomState: row?.roomState || row?.status || "active",
    cleanupState: row?.cleanupState || row?.status || "active",
    stateStatus: row?.stateStatus || row?.status || "active",
    workspace: "member-command",
    visibility: "member",
    kind: "deal",
    roomType: "deal",
...
function readDealsFrom(key: string) {
  return readArray(key)
    .map(normalizeDeal)
    .filter((row) => row.id && row.title && row.title !== "Untitled Room" && row.title !== "Untitled Deal Room");
}

function readCanonicalDeals() {
  const all = [
    ...readDealsFrom(CANONICAL_STORE_KEY),
    ...readDealsFrom(LEGACY_STORE_KEY),
    ...readDealsFrom(MY_ROOMS_STORE_KEY),
    ...readDealsFrom(COMMAND_DEAL_STORE_KEY),
    ...readDealsFrom(MEMBER_ROOMS_STORE_KEY),
  ];

  const map = new Map<string, DealRoom>();
  all.forEach((deal) => {
    const previous = map.get(deal.id);
    if (!previous || previous.updatedAt < deal.updatedAt) map.set(deal.id, deal);
  });
...
function dealIntel(room: DealRoom) {
  ...
  const analyzer = `Deal room: ${room.title}. ${room.assetClass} • ${room.propertyType} • ${room.strategy.join(", ")}. ${locationFor(room)}. Ask ${room.askingPrice || "N/A"}, value ${room.propertyValue || "N/A"}, repairs ${room.repairs || "N/A"}. Signal: ${signal}. Strength ${score}%. Risk ${risk}%. Urgency ${urgency}%. Next move: verify control, title, access, proof, photos, and route to ${room.routeTo.join(", ")}.`;
...
function saveDeal(room: DealRoom) {
  ...
  const photos = safeList(room.photos || room.photoUrls);
  const cover = safeText(room.coverPhoto || room.photoUrl || room.imageUrl || photos[0]);
  const intel = dealIntel({ ...room, id, roomId: id, photos, photoUrls: photos, coverPhoto: cover, photoUrl: cover, imageUrl: cover });

  const next: DealRoom = normalizeDeal({
    ...room,
    ...owner,
    id,
    roomId: id,
    title: safeText(room.title),
    photos,
    photoUrls: photos,
    coverPhoto: cover,
    photoUrl: cover,
...
function stopKeys(e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) {
  e.stopPropagation();
}

function Field({ title, value, onChange }: { title: string; value: string; onChange: (value: string) => void }) {
  return <label><div style={label}>{title}</div><input style={input} value={value} onKeyDownCapture={stopKeys} onKeyUpCapture={stopKeys} onChange={(e) => onChange(e.target.value)} /></label>;
}

function TextArea({ title, value, onChange }: { title: string; value: string; onChange: (value: string) => void }) {
  return <label><div style={label}>{title}</div><textarea style={textarea} value={value} onKeyDownCapture={stopKeys} onKeyUpCapture={stopKeys} onChange={(e) => onChange(e.target.value)} /></label>;
}

function SelectField({ title, value, options, onChange }: { title: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label><div style={label}>{title}</div><select style={input} value={value} onChange={(e) => onChange(e.target.value)}>{options.map((o) => <option key={o}>{o}</option>)}</select></label>;
}

function ChipSet({ title, options, selected, onToggle }: { title: string; options: string[]; selected: string[]; onToggle: (value: string) => void }) {
  return <div><div style={label}>{title}</div><div style={row}>{options.map((o) => <button key={o} type="button" style={selected.includes(o) ? goldBtn : btn} onClick={() => onToggle(o)}>{o}</button>)}</div></div>;
}

function Meter({ title, value }: { title: string; value: number }) {
  return <div style={panel}><div style={eyebrow}>{title}</div><h2 style={h2}>{value}%</h2><div style={{ height: 10, background: "#070a12", borderRadius: 999, overflow: "hidden" }}><div style={{ width: `${value}%`, height: "100%", background: "#ffdc68" }} /></div></div>;
}
...
export default function DealCreatePage() {
  ...
  function toggle(key: keyof DealRoom, value: string) {
    const set = new Set(safeList(form[key]));
  ...
  async function addPhotos(files: FileList | null) {
    ...
    setForm({
      ...
      photos: merged,
      photoUrls: merged,
      coverPhoto: form.coverPhoto || merged[0] || "",
      photoUrl: form.photoUrl || merged[0] || "",
      imageUrl: form.imageUrl || merged[0] || "",
    });
  }
      ...
      <div style={wrap}>
        ...
        <section style={sticky}>
          <div style={row}>
            ...
            <span style={muted}>{safeText(form.title, "No title yet")} • {form.assetClass} • {locationFor(form)} • Photos {form.photos.length}</span>
          </div>
        </section>
        ...
        {banner ? (
          <section style={activePanel}>
            ...
            <div style={{ ...row, marginTop: 18 }}>
              <Link href={`/deal-rooms/${encodeURIComponent(savedId)}`} style={goldBtn}>Open Room</Link>
              <Link href="/command" style={btn}>Command</Link>
              <Link href="/my-rooms" style={btn}>My Rooms</Link>
              <button type="button" style={btn} onClick={() => { setBanner(""); setSavedId(""); setForm(defaultDeal()); }}>Create Another</button>
            </div>
          </section>
        ) : null}
        ...
        <Section title="AI Deal Preview">
          <div style={grid}>
            ...
            <Meter title="Urgency" value={intel.urgency} />
            <div style={panel}><div style={eyebrow}>Signal</div><p style={sub}>{intel.signal}</p><p style={muted}>Estimated spread: {intel.spread ? `$${intel.spread.toLocaleString()}` : "needs ask/value/repairs"}</p></div>
          </div>
        </Section>
        ...
        <Section title="Asset + Strategy">
          ...
          <div style={{ height: 18 }} />
          <ChipSet title="Strategy" options={STRATEGIES} selected={form.strategy} onToggle={(v) => toggle("strategy", v)} />
          <div style={{ height: 18 }} />
          <ChipSet title="Route To" options={ROUTES} selected={form.routeTo} onToggle={(v) => toggle("routeTo", v)} />
        </Section>

        <Section title="Property + Market">
          <div style={grid}>
            <Field title="Deal Title" value={form.title} onChange={(v) => update("title", v)} />
            <SelectField title="State" value={form.state} options={STATES} onChange={(v) => update("state", v)} />
            <Field title="City" value={form.city} onChange={setCity} />
            <Field title="County" value={form.county} onChange={(v) => update("county", v)} />
            <Field title="Address / Location" value={form.address} onChange={(v) => update("address", v)} />
            <SelectField title="Property Type" value={form.propertyType} options={propertyTypes} onChange={(v) => update("propertyType", v)} />
          </div>
        </Section>

        <Section title="Deal Control + Proof">
          <div style={grid}>
            <SelectField title="Condition" value={form.condition} options={CONDITION} onChange={(v) => update("condition", v)} />
            <SelectField title="Occupancy" value={form.occupancy} options={OCCUPANCY} onChange={(v) => update("occupancy", v)} />
            <SelectField title="Control Status" value={form.controlStatus} options={CONTROL} onChange={(v) => update("controlStatus", v)} />
            <SelectField title="Timeline" value={form.timeline} options={TIMELINE} onChange={(v) => update("timeline", v)} />
            <SelectField title="Proof Available" value={form.proofStatus} options={YESNO} onChange={(v) => update("proofStatus", v)} />
            <SelectField title="Access Available" value={form.accessStatus} options={YESNO} onChange={(v) => update("accessStatus", v)} />
            <SelectField title="Title Status" value={form.titleStatus} options={["Unknown", "Clear", "Lien", "Probate", "Dispute", "Needs Review"]} onChange={(v) => update("titleStatus", v)} />
          </div>
        </Section>

        <Section title="Numbers">
          <div style={grid}>
            <Field title="Ask Price" value={form.askingPrice} onChange={(v) => update("askingPrice", v)} />
            <Field title="Value / ARV" value={form.propertyValue} onChange={(v) => update("propertyValue", v)} />
            <Field title="Repairs / Work" value={form.repairs} onChange={(v) => update("repairs", v)} />
          </div>
        </Section>
        ...
        {form.assetClass === "Residential" ? (
          <Section title="Residential Details">
            <div style={grid}>
              <Field title="Beds" value={form.beds} onChange={(v) => update("beds", v)} />
              <Field title="Baths" value={form.baths} onChange={(v) => update("baths", v)} />
              <Field title="Sqft" value={form.sqft} onChange={(v) => update("sqft", v)} />
              <Field title="Monthly Rent Estimate" value={form.monthlyRent} onChange={(v) => update("monthlyRent", v)} />
            </div>
          </Section>
        ) : null}
        ...
        {form.assetClass === "Commercial" ? (
          <Section title="Commercial Details">
            <div style={grid}>
              <Field title="NOI / Income" value={form.noi} onChange={(v) => update("noi", v)} />
              <Field title="Units / Tenants" value={form.units} onChange={(v) => update("units", v)} />
              <Field title="Cap Rate" value={form.capRate} onChange={(v) => update("capRate", v)} />
              <SelectField title="Leases In Place" value={form.leasesInPlace} options={YESNO} onChange={(v) => update("leasesInPlace", v)} />
              <SelectField title="Tenant Quality" value={form.tenantQuality} options={["Unknown", "Strong", "Average", "Weak", "Vacant"]} onChange={(v) => update("tenantQuality", v)} />
            </div>
          </Section>
        ) : null}
        ...
        {form.assetClass === "Land" ? (
          <Section title="Land Details">
            <div style={grid}>
              <Field title="Acres" value={form.acres} onChange={(v) => update("acres", v)} />
              <Field title="Zoning" value={form.zoning} onChange={(v) => update("zoning", v)} />
              <Field title="Road Frontage" value={form.roadFrontage} onChange={(v) => update("roadFrontage", v)} />
              <SelectField title="Utilities" value={form.utilities} options={["Unknown", "Available", "Nearby", "Not Available"]} onChange={(v) => update("utilities", v)} />
              <SelectField title="Entitlement Status" value={form.entitlementStatus} options={["Unknown", "None", "In Process", "Approved", "Denied"]} onChange={(v) => update("entitlementStatus", v)} />
              <SelectField title="Survey Available" value={form.surveyAvailable} options={YESNO} onChange={(v) => update("surveyAvailable", v)} />
              <SelectField title="Soil Test" value={form.soilTest} options={YESNO} onChange={(v) => update("soilTest", v)} />
              <SelectField title="Topo Available" value={form.topoAvailable} options={YESNO} onChange={(v) => update("topoAvailable", v)} />
              <SelectField title="Access Road" value={form.accessRoad} options={YESNO} onChange={(v) => update("accessRoad", v)} />
            </div>
          </Section>
        ) : null}
        ...
        <Section title="Contact + AI Notes">
          <div style={grid}>
            <Field title="Contact Name" value={form.contactName} onChange={(v) => update("contactName", v)} />
            <Field title="Phone" value={form.contactPhone} onChange={(v) => update("contactPhone", v)} />
            <Field title="Email" value={form.contactEmail} onChange={(v) => update("contactEmail", v)} />
            <SelectField title="Best Contact" value={form.bestContact} options={CONTACT} onChange={(v) => update("bestContact", v)} />
            <TextArea title="Notes / AI Context" value={form.notes} onChange={(v) => update("notes", v)} />
          </div>
        </Section>
        ...
        <Section title="Photos">
          ...
          {form.photos.length ? (
            <div style={grid}>
              {form.photos.map((p, i) => (
                <div key={`${p.slice(0, 20)}-${i}`} style={panel}>
                  <img src={p} alt={`Deal ${i + 1}`} style={photoStyle} />
                  <button type="button" style={redBtn} onClick={() => removePhoto(i)}>Delete Photo</button>
                </div>
              ))}
            </div>
          ) : null}
