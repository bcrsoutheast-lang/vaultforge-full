...
export default function DealCreatePage() {
  ...
  function saveDeal(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const raw = window.localStorage.getItem("vaultforge_clean_deal_rooms");
      const existing = raw ? JSON.parse(raw) : [];
      const safeExisting = Array.isArray(existing) ? existing : [];
      window.localStorage.setItem("vaultforge_clean_deal_rooms", JSON.stringify([deal, ...safeExisting]));
      window.localStorage.setItem(`vaultforge_clean_deal_room_${deal.id}`, JSON.stringify(deal));
      setSavedId(deal.id);
      showToast("success", "Deal saved", "Deal Room created. Opening Deal Rooms is now available below.");
    } catch (error) {
        ...
        <form onSubmit={saveDeal}>
          ...
          <section style={card}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 18 }}>
              <button type="submit" onClick={() => saveDeal()} disabled={saving} style={primaryButton}>{saving ? "Saving..." : "Save Deal Room"}</button>
