...
export default function DealCreatePage() {
  ...
  function toggle(key: "routeTo" | "strategy" | "motivation" | "documentsAvailable", value: string) {
  ...
  function analyzer() {
    const route = list(form.routeTo).join(", ") || "Buyer";
    const strategy = list(form.strategy).join(", ") || "No strategy";
    const facts = `${txt(form.assetClass)} ${txt(form.propertyType)} in ${loc(form)}.`;
    const numbers = `Ask ${txt(form.askingPrice, "N/A")}, value ${txt(form.propertyValue, "N/A")}, repairs ${txt(form.repairs, "N/A")}.`;
    return `Deal analyzer: ${facts} ${numbers} Strategy: ${strategy}. Route to: ${route}. Next move: verify control, access, title, numbers, and route to matching member profiles.`;
  }
  ...
  function save() {
    ...
    if (!id) {
      setError("Deal did not save. Browser storage may be full. Delete extra photos or old test data and try again.");
      return;
    }

    setSavedId(id);
    setBanner("Deal saved. It is now active in Command, Network, Deal Rooms, Alerts, and matching.");
  }
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <section style={hero}>
          ...
          <h1 style={h1}>Opportunity form.</h1>
          <p style={sub}>Residential, Commercial, and Land now switch fields. Photos can be previewed and deleted before save.</p>
        </section>

        <Section title="Live Preview">
          <div style={activePanel}>
            {photos[0] ? <img src={photos[0]} alt="Deal preview" style={photoStyle} /> : null}
            <div style={eyebrow}>{txt(form.assetClass, "Residential")} • {txt(form.propertyType, "Single Family")}</div>
            <h2 style={h2}>{txt(form.title, "Untitled Deal")}</h2>
            <p style={sub}>{loc(form)}</p>
            <p style={muted}>Route: {list(form.routeTo).join(", ") || "None"} • Strategy: {list(form.strategy).join(", ") || "None"}</p>
          </div>
        </Section>
        ...
        <Section title="Numbers">
          <div style={grid}>
            ...
            {txt(form.assetClass) === "Commercial" ? (
              <>
                <Field label="NOI" value={txt(form.noi)} onChange={(value) => up("noi", value)} />
              ...
              <>
                ...
                <Field label="Road Frontage" value={txt(form.roadFrontage)} onChange={(value) => up("roadFrontage", value)} />
              </>
            ) : (
              <>
        ...
        {txt(form.assetClass) === "Land" ? (
          <Section title="Land Fields">
            <div style={grid}>
              ...
              <Field label="Lot Size" value={txt(form.lotSize)} onChange={(value) => up("lotSize", value)} />
            </div>
          </Section>
        ) : null}
