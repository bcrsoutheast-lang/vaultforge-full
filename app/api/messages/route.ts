...
async function insertOne(supabase: any, table: string, payload: AnyRow) {
  try {
    const { data, error } = await supabase.from(table).insert(stripNulls(payload)).select("*").single();

    if (!error && data) {
      return { ok: true, table, data, keys: Object.keys(stripNulls(payload)) };
    }

    return { ok: false, table, error: error?.message || `Insert failed for ${table}`, keys: Object.keys(stripNulls(payload)) };
  } catch (error: any) {
    return { ok: false, table, error: error?.message || String(error), keys: Object.keys(stripNulls(payload)) };
  }
}

async function insertWithVariants(supabase: any, table: string, variants: AnyRow[]) {
  ...
  return { ok: false, table, attempts, error: attempts.map((item) => `${item.table}: ${item.error}`).join(" | ") };
}
...
export async function POST(request: Request) {
  try {
    ...
    /*
      IMPORTANT:
      We do NOT send custom id first.
      Many Supabase tables use uuid id defaults and reject msg_... strings.
      Start with schema-safe payloads, then gradually try wider field sets.
    */
    ...
    for (const table of messageTables) {
      const result = await insertWithVariants(supabase, table, [
    ...
    /*
      Final safety fallback: save as activity event.
      This prevents the UI from losing the member's request if message tables are missing/misaligned.
    */
    const fallbackTables = ["vf_activity_events", "activity_events"];

    for (const table of fallbackTables) {
      const result = await insertWithVariants(supabase, table, [activityFallbackPayload]);
      ...
      if (result.ok) {
        return NextResponse.json({
          ...
          message: result.data,
          thread_id: threadId,
          message_id: result.data.id || result.data.event_id || messageId,
          from_email: fromEmail,
          to_email: toEmail,
          self_message: selfMessage,
          links: {
