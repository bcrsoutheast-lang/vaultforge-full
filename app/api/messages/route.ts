...
function normalizeMessage(row: AnyRow, table: string) {
  ...
  return {
    ...row,
    id: first(row.id, row.message_id, row.event_id, metadata.message_id, `${table}-${row.created_at || ""}-${fromEmail}`),
    message_id: first(row.message_id, row.id, row.event_id, metadata.message_id),
...
async function selectRecent(supabase: any, table: string) {
  ...
  for (const column of orderColumns) {
    try {
      const { data, error } = await supabase.from(table).select("*").order(column, { ascending: false }).limit(300);
