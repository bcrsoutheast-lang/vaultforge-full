...
async function selectRecent(supabase: any, table: string, limit = 250) {
...
export async function GET(request: Request) {
  try {
    ...
    const exactRows: AnyRow[] = [];

    for (const table of tables) {
      const rows = await selectRecent(supabase, table, 260);
      for (const row of rows) {
        if (!dealish(row, table)) continue;
        if (!matchesId(row, id)) continue;
        exactRows.push(normalizeDeal(row, table));
      }
    }
    ...
    const seed = exactRows.sort((a, b) => completenessScore(b) - completenessScore(a))[0];
    const canonical = first(seed._dedupe_key, seed.canonical_project_key, seed.canonical_event_id, seed.deal_id, seed.id);

    const relatedRows: AnyRow[] = [...exactRows];

    for (const table of tables) {
      const rows = await selectRecent(supabase, table, 260);
      for (const row of rows) {
        if (!dealish(row, table)) continue;
        const normalized = normalizeDeal(row, table);
        const key = first(normalized._dedupe_key, normalized.canonical_project_key, normalized.canonical_event_id, normalized.deal_id, normalized.id);
        if (key && canonical && key === canonical) relatedRows.push(normalized);
      }
    }

    const uniqueBySourceAndId = new Map<string, AnyRow>();
    for (const row of relatedRows) {
      uniqueBySourceAndId.set(`${row._source_table || row.source_table}-${row.id}-${row.canonical_event_id}`, row);
    }

    let merged = Array.from(uniqueBySourceAndId.values()).sort((a, b) => {
      const scoreDiff = completenessScore(b) - completenessScore(a);
      if (scoreDiff !== 0) return scoreDiff;
      return Number(a._source_rank || 9) - Number(b._source_rank || 9);
    })[0];

    for (const row of Array.from(uniqueBySourceAndId.values())) {
      if (row === merged) continue;
      merged = mergeDeal(merged, row);
    }
    ...
    return NextResponse.json({
      ...
      detail_model: "merged vf_deals + routing mirrors by canonical project key",
    });
  } catch (error: any) {
    return NextResponse.json(
      { ok: false, error: "Could not load deal.", details: error?.message || String(error) },
      { status: 500 }
    );
  }
}
