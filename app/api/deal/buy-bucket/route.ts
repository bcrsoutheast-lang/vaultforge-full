...
export async function POST(req: Request) {
  ...
  if (!memberEmail) return NextResponse.json({ error: "Not logged in." }, { status: 401 });
  ...
  const res = await fetch(`${config.url}/rest/v1/vf_buy_bucket`, {
    ...
    body: JSON.stringify({ member_email: memberEmail, deal_id: dealId }),
  });

  if (res.ok) return NextResponse.json({ ok: true, status: "saved" });
  ...
  return NextResponse.json({ error: "Failed to add to Buy Bucket.", details }, { status: 500 });
}
