...
function ownerEmail(row: Row) {
  const m = metadataOf(row);
  return cleanEmail(first(row.owner_email, row.member_email, row.created_by_email, row.submitted_by_email, row.user_email, row.email, m.owner_email, m.member_email, m.created_by_email, m.submitted_by_email, m.user_email, m.email));
}
...
function normalizeDeal(row: Row) {
  ...
  return {
    ...
    member_email: cleanEmail(first(row.member_email, row.owner_email, row.created_by_email, row.submitted_by_email, m.member_email, m.owner_email, m.created_by_email, m.submitted_by_email)),
    ...
    created_at: first(row.created_at, row.updated_at, m.created_at, m.updated_at, new Date().toISOString()),
    source_table: TABLE,
    source: "deal_feed",
  };
}
