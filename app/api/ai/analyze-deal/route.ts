...
export function analyzeDeal(input: any) {
  ...
  if (description.includes("fund") || description.includes("loan") || description.includes("lender")) {
  ...
  const riskSummary = Array.from(new Set(risk)).join("; ") || "standard underwriting needed before commitment";
  ...
  const ai_summary =
    ...
    `Lender relevance: ${price ? `price point at $${Number(price).toLocaleString()} should be reviewed for funding fit` : "funding fit depends on final price and terms"}. ` +
