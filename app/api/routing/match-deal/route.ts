...
function arrayContains(values: unknown, target: string) {
  ...
  return values.map((v) => String(v || "")).includes(target);
}

function priceMatches(member: any, dealPrice: number | null) {
  if (dealPrice === null || Number.isNaN(dealPrice)) return true;

  const min = member.min_price === null || member.min_price === undefined ? null : Number(member.min_price);
  const max = member.max_price === null || member.max_price === undefined ? null : Number(member.max_price);
...
function roleIsRelevant(role: string, deal: any) {
  ...
  if (role === "Contractor") return strategy === "Fix & Flip" || String(deal.description || "").toLowerCase().includes("repair") || String(deal.description || "").toLowerCase().includes("rehab");
  if (role === "Developer") return strategy === "Development" || propertyType === "Land" || propertyType === "Commercial";
