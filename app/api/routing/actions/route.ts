...
function inferRoleMatch(action: string, targetRole: string) {
  ...
  if (action.includes("jv")) return "JV Partner";

  return "Manual Review";
}

function inferUrgency(priority: string, action: string) {
  ...
  if (p === "high" || action === "high_priority") {
    return "High-value routing or relationship opportunity identified.";
  }

  if (action === "needs_review") {
    return "Owner review needed before routing can continue.";
  }

  if (action === "watch") {
    return "Watchlist item being monitored before further action.";
  }
...
function inferConfidence({
  ...
  if (priority.toLowerCase() === "urgent") score += 12;
  if (priority.toLowerCase() === "high") score += 8;

  if (action.includes("route_to")) score += 8;
  if (action === "watch") score -= 6;
...
function normalizedAction(body: AnyRecord, adminEmail: string) {
  ...
  const roleMatch = first(
    body.role_match,
    body.roleMatch,
    inferRoleMatch(action, targetRole)
  );

  const priority = first(body.priority, action === "high_priority" ? "high" : "medium");
  const urgencyReason = first(
    body.urgency_reason,
    body.urgencyReason,
    body.routing_reason,
    body.routingReason,
    inferUrgency(priority, action)
  );

  const confidenceScore = clampScore(
    body.confidence_score || body.confidenceScore || body.match_score || body.matchScore,
    inferConfidence({
      stateMatch,
      strategyMatch,
      roleMatch,
  ...
  const routingSummary = first(
    ...
    [
      ...
      .filter(Boolean)
      .join(" · ") || "General operational alignment"
  );
  ...
  return {
    ...
    target_role: targetRole || roleMatch || null,
    ...
    urgency_reason: urgencyReason || null,
    routing_reason: urgencyReason || null,
...
function normalizeRow(row: AnyRecord) {
  const action = first(row.action, row.routing_action);
  const priority = first(row.priority, "medium");
  const stateMatch = first(row.state_match, row.market_match);
  const strategyMatch = first(row.strategy_match);
  const roleMatch = first(row.role_match, row.target_role);
  const confidenceScore = clampScore(
    first(row.confidence_score, row.match_score),
    inferConfidence({
      stateMatch,
      strategyMatch,
      roleMatch,
  ...
  return {
    ...
    market_match: stateMatch,
    strategy_match: strategyMatch,
    role_match: roleMatch,
    urgency_reason: first(row.urgency_reason, row.routing_reason) || inferUrgency(priority, action),
    routing_reason: first(row.routing_reason, row.urgency_reason) || inferUrgency(priority, action),
    routing_summary:
      first(row.routing_summary) ||
      [
        stateMatch ? `State fit: ${stateMatch}` : "",
        strategyMatch ? `Strategy fit: ${strategyMatch}` : "",
        roleMatch ? `Role fit: ${roleMatch}` : "",
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      table: ACTION_TABLE,
      keys: result.keys,
      message: "Routing action logged safely with context.",
    });
  } catch (error: any) {
    return NextResponse.json(
      {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      counts: {
        ...
        buyer: actions.filter((item) => item.action === "route_to_buyer").length,
        lender: actions.filter((item) => item.action === "route_to_lender").length,
        operator: actions.filter((item) => item.action === "route_to_operator").length,
      },
      note: owner
        ? "Owner/global routing action log with context."
