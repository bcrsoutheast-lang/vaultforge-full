...
import { clearVaultForgeEmail, getVaultForgeEmail, vaultForgeHeaders } from "../lib/vaultforge-client-auth";

type Summary = {
  email: string;
  profileComplete: boolean;
  counts: { activeDeals: number; myDeals: number; buyBucket: number; alerts: number; messages: number; members: number };
};
...
function ActionCard({ href, label, title, description, count }: { href: string; label: string; title: string; description: string; count?: number | string }) {
  return <Link href={href} style={cardStyle}><span style={pillStyle}>{label}</span><h2 style={{ fontSize: 28, margin: "0 0 8px" }}>{title}</h2>{count !== undefined && <div style={countStyle}>{count}</div>}<p style={{ color: "rgba(255,255,255,.68)", fontSize: 17, lineHeight: 1.45 }}>{description}</p></Link>;
}
...
export default function Dashboard() {
  ...
  async function loadSummary() {
    ...
    try {
      const res = await fetch("/api/dashboard/summary", { cache: "no-store", headers: vaultForgeHeaders() });
  ...
  return (
    <main style={shellStyle}>
      <nav style={navStyle}>
        <Link href="/profile" style={navLinkStyle}>Profile</Link><Link href="/submit" style={navLinkStyle}>Create Deal</Link><Link href="/projects" style={navLinkStyle}>Projects</Link><Link href="/buy-bucket" style={navLinkStyle}>Buy Bucket</Link><Link href="/alerts" style={navLinkStyle}>Alerts</Link><Link href="/messages" style={navLinkStyle}>Messages</Link><Link href="/network" style={navLinkStyle}>Network</Link><button onClick={logout} style={{ ...navLinkStyle, background: "transparent", cursor: "pointer" }}>Logout</button>
      </nav>

      <section style={heroStyle}>
        <div style={{ textAlign: "center", marginBottom: 18 }}><img src="/vaultforge-logo.png" alt="VaultForge" style={{ width: "100%", maxWidth: 420, borderRadius: 18 }} /></div>
      ...
      {error && <section style={{ ...cardStyle, color: "#ffd0d0", borderColor: "rgba(255,107,107,.55)" }}>{error}<div style={{ marginTop: 16 }}><Link href="/login" style={navLinkStyle}>Login again</Link></div></section>}

      {!loading && !error && summary && (
        <>
          {!summary.profileComplete && <section style={{ ...heroStyle, borderColor: "rgba(157,243,191,.45)", background: "rgba(157,243,191,.07)" }}><p style={{ color: "#9df3bf", letterSpacing: 4, fontWeight: 800 }}>NEXT BEST ACTION</p><h2 style={{ fontSize: 34, margin: "0 0 14px" }}>Complete Profile + Buy Box</h2><p style={{ color: "rgba(255,255,255,.72)", fontSize: 19, lineHeight: 1.45 }}>This powers AI routing, network visibility, lender matching, and alert quality.</p><Link href="/profile" style={navLinkStyle}>Open Profile</Link></section>}
