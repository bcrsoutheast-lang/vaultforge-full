...
async function loadRoutingRows(email: string, owner: boolean) {
...
export default function RoutingInboxPage() {
  ...
  async function load() {
    ...
    try {
      const loaded = await loadRoutingRows(currentEmail, currentOwner);
      setRows(loaded);
      setStatus(loaded.length ? "" : "No routing actions found yet.");
    } catch (error: any) {
      setRows([]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) =>
      [titleOf(row), noteOf(row), marketOf(row), priorityOf(row), statusOf(row), signalIdOf(row), itemIdOf(row), ownerEmailOf(row), typeOf(row)]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
      ...
      <div style={styles.wrap}>
        <nav style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <Link href="/introductions" style={styles.secondary}>Introductions</Link>
            <Link href="/messages" style={styles.secondary}>Messages</Link>
          </div>
        </nav>

        <section style={styles.hero}>
          <p style={styles.eyebrow}>VaultForge Routing</p>
          <h1 style={styles.title}>Routing windows.</h1>
          <p style={{ ...styles.muted, maxWidth: 820, fontSize: 18, marginTop: 16 }}>
            Same card layout and same working Message Owner flow. Routing stays visual, clean, and connected.
          </p>

          <div style={{ marginTop: 16 }}>
            <span style={styles.chip}>Signed in: {email || "unknown"}</span>
            <span style={styles.chip}>Routes: {rows.length}</span>
          ...
          <div className="vf-actions" style={styles.actionRow}>
            <button type="button" onClick={load} style={styles.primary}>Refresh Routing</button>
            <Link href="/introductions" style={styles.secondary}>Introductions</Link>
            <Link href="/messages" style={styles.secondary}>Messages</Link>
            <Link href="/alerts" style={styles.secondary}>Alerts</Link>
          </div>
        ...
        <section style={styles.hero}>
          <p style={styles.eyebrow}>Search Routing</p>
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search routing, market, owner, signal id..." style={styles.input} />
        </section>
        ...
        <section style={styles.grid}>
          {filtered.map((row, index) => (
            <WindowCard key={`${signalIdOf(row)}-${itemIdOf(row)}-${index}`} row={row} email={email} mode="routing" />
          ))}
        </section>
      </div>
    </main>
