...
function itemState(item: any) {
  return String(item.state || item.propertyState || item.dealState || item.painState || item.locationState || "").trim().toUpperCase();
}

function saveRequest(kind: string, item: any, message: string) {
  let rows: any[] = [];
  try { const parsed = JSON.parse(localStorage.getItem(INVESTOR_REQUESTS_KEY) || "[]"); rows = Array.isArray(parsed) ? parsed : []; } catch {}
...
function TeaserCard({ kind, item }: { kind: "Deal" | "Pain"; item: any }) {
  ...
  return (
    <div style={panel}>
      <div style={eyebrow}>{kind} • {state}</div>
      ...
      <p style={{ ...muted, marginTop: 14 }}>Member information, seller information, exact private notes, and contact details are hidden until the member/admin approves deeper access.</p>
      <label style={field}><span style={eyebrow}>Request Message</span><textarea style={{ ...input, minHeight: 100 }} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="I am interested. Send more info..." /></label>
      <div style={{ ...row, marginTop: 12 }}>
        <button type="button" style={goldBtn} onClick={() => { saveRequest(kind, item, message || "Investor requested more information."); setSent(true); }}>Request More Info</button>
      </div>
...
export default function InvestorRoomPage() {
  ...
  useEffect(() => {
    ...
    const refresh = () => { setInvestor(readInvestor()); setTick((x) => x + 1); };
  ...
  const deals = useMemo(() => readRows(["vaultforge_clean_deal_rooms", "vaultforge_deal_rooms", "vaultforge_rooms_deals"]).filter((item) => itemState(item) === state), [state, tick]);
  const pains = useMemo(() => readRows(["vaultforge_clean_pain_rooms", "vaultforge_clean_pain_rooms_v1", "vaultforge_clean_pain_rooms_v2", "vaultforge_pain_rooms"]).filter((item) => itemState(item) === state), [state, tick]);

  if (!active) {
    return (
      <main style={page}><div style={wrap}><section style={hero}>
        <div style={eyebrow}>Investor Room Locked</div>
        <h1 style={h1}>Approval and payment required.</h1>
        <p style={sub}>Complete investor application and payment before entering the investor room.</p>
        <div style={{ ...row, marginTop: 18 }}><Link href="/investor-application" style={goldBtn}>Investor Application</Link><Link href="/investor-payment" style={btn}>Investor Payment</Link></div>
      </section></div></main>
    );
  }
    ...
    <main style={page}>
      <div style={wrap}>
        <section style={hero}>
          ...
          <p style={sub}>Browse limited state teaser cards and request information through VaultForge. No private member data is exposed.</p>
        </section>

        <section style={card if False else panel}>
          <div style={eyebrow}>States</div>
          <div style={row}>{STATES.map((s) => <button key={s} type="button" style={s === state ? goldBtn : btn} onClick={() => setState(s)}>{s}</button>)}</div>
        </section>

        <section style={{ marginTop: 18 }}>
          <div style={eyebrow}>Deal Cards • {state}</div>
          <div style={grid}>{deals.length ? deals.map((item, index) => <TeaserCard key={`deal-${index}`} kind="Deal" item={item} />) : <div style={panel}><h2 style={h2}>No deal teasers yet.</h2><p style={sub}>Approved Deal cards for this state will appear here.</p></div>}</div>
        </section>

        <section style={{ marginTop: 22 }}>
          <div style={eyebrow}>Pain Cards • {state}</div>
          <div style={grid}>{pains.length ? pains.map((item, index) => <TeaserCard key={`pain-${index}`} kind="Pain" item={item} />) : <div style={panel}><h2 style={h2}>No pain teasers yet.</h2><p style={sub}>Approved Pain cards for this state will appear here.</p></div>}</div>
        </section>

        <section style={{ ...hero, marginTop: 24 }}>
          <div style={eyebrow}>Network Capabilities</div>
          <p style={sub}>Funding, contractors, operators, title/closing, and execution support are available through the private member network after approval by members/admin.</p>
        </section>
      </div>
    </main>
  );
