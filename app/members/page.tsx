  ...
  return (
    <main style={page}>
      ...
      <div style={wrap}>
        ...
        <section style={{ ...hero, borderColor: "rgba(157,243,191,.24)" }}>
          <div style={greenEyebrow}>Member Network by State</div>
          <h2 style={{ fontSize: "clamp(34px,8vw,66px)", lineHeight: 0.95, margin: "0 0 12px" }}>
            Southeast operator map.
          </h2>
          <p style={{ ...muted, fontSize: 18 }}>
            Click a state to filter the private member network by market coverage.
          </p>

          <div style={{ ...grid, gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))" }}>
            <button
              type="button"
              ...
              style={{
                ...
                cursor: "pointer",
                textAlign: "left",
                borderColor: !stateFilter ? "rgba(245,217,120,.75)" : "rgba(255,255,255,.14)",
              }}
            >
              <div style={greenEyebrow}>ALL</div>
              <div style={{ fontSize: 42, fontWeight: 950 }}>{realMembers.length}</div>
              <p style={muted}>All member markets</p>
            </button>
            ...
            {stateCounts.map((state) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  textAlign: "left",
                  borderColor: stateFilter === state.short ? "rgba(245,217,120,.75)" : "rgba(157,243,191,.24)",
                }}
              >
                <div style={greenEyebrow}>{state.short}</div>
                <div style={{ fontSize: 42, fontWeight: 950 }}>{state.count}</div>
                <p style={muted}>{state.label}</p>
              </button>
            ))}
          </div>
          ...
          {stateFilter && (
            <p style={{ ...muted, marginTop: 16 }}>
              Showing members active in <strong style={{ color: "#9df3bf" }}>{stateFilter}</strong>.
              <button type="button" onClick={() => setStateFilter("")} style={{ ...ghost, marginLeft: 10 }}>
                Clear State Filter
              </button>
            </p>
          )}
