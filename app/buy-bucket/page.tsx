...
export default function BuyBucketPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setDeals((data?.deals || []).map((item: any) => item.deal || item.vf_deals || item));
      setStatus("");
    } catch (err: any) { setStatus(err?.message || "Could not load Buy Bucket."); }
  }
  useEffect(() => { load(); }, []);
  return <main style={shell}><div style={wrap}>
    <section style={hero}><div style={eyebrow}>BUY BUCKET</div><h1 style={{ fontSize: "clamp(56px,12vw,96px)", lineHeight: .9, margin: "0 0 18px" }}>Saved acquisition targets.</h1><Link href="/dashboard" style={ghost}>Dashboard</Link><Link href="/projects" style={btn}>Projects</Link><Link href="/submit" style={ghost}>Create</Link><button onClick={load} style={btn}>Refresh</button></section>
