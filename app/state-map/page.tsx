...
function Nav() {
  return (
    <nav style={nav}>
      ...
      <Link href="/state-map" style={goldBtn}>State Map</Link>
...
export default function StateMapPage() {
  ...
  const open = stateStats(openState, deals, pains, threads);
    ...
    <main style={page}>
      ...
      <div style={wrap}>
        ...
        <section style={hero}>
          ...
          <h1 style={h1}>Market intelligence by state.</h1>
          <p style={sub}>Click a state to drill into active Opportunity rooms, Pain rooms, unread signals, and pressure scores.</p>
        </section>

        <Section title="State Command Cards">
          <div style={grid}>
            {STATES.map((state) => {
              const stats = stateStats(state, deals, pains, threads);
              const isOpen = openState === state;
              const style = stats.hot ? (stats.unreadPains.length ? pulseRed : pulseGold) : isOpen ? activePanel : panel;
              return (
                <button key={state} type="button" style={style} onClick={() => setOpenState(state)}>
                  <div style={eyebrow}>{state}</div>
                  <h2 style={h2}>{stats.stateDeals.length + stats.statePains.length}</h2>
                  <p style={muted}>Deals {stats.stateDeals.length} • Pain {stats.statePains.length}</p>
                  <p style={muted}>Unread: Deal {stats.unreadDeals.length} • Pain {stats.unreadPains.length} • Msg {stats.unreadMessages.length}</p>
                  <p style={muted}>Pressure {stats.pressure}% • Opportunity {stats.opportunity}%</p>
                </button>
              );
            })}
          </div>
        ...
        <Section title={`${openState} Intelligence Snapshot`}>
          <div style={grid}>
            <Meter title="Pressure Score" value={open.pressure} />
            <Meter title="Opportunity Score" value={open.opportunity} />
            <div style={open.unreadPains.length ? pulseRed : panel}>
              <div style={eyebrow}>Pain Signals</div>
              <h2 style={h2}>{open.statePains.length}</h2>
              <p style={muted}>{open.unreadPains.length} unread/new</p>
            </div>
            <div style={open.unreadDeals.length ? pulseGold : panel}>
              <div style={eyebrow}>Opportunity Signals</div>
              <h2 style={h2}>{open.stateDeals.length}</h2>
              <p style={muted}>{open.unreadDeals.length} unread/new</p>
            </div>
            <div style={open.unreadMessages.length ? pulseRed : panel}>
              <div style={eyebrow}>Messages</div>
              <h2 style={h2}>{open.stateMessages.length}</h2>
              <p style={muted}>{open.unreadMessages.length} unread</p>
            </div>
          </div>
        </Section>

        <Section title={`${openState} Opportunity Rooms`}>
          {open.stateDeals.length ? <div style={grid}>{open.stateDeals.map((room) => <RoomCard key={rid(room)} room={room} kind="deal" />)}</div> : <p style={sub}>No active opportunity rooms in {openState}.</p>}
        </Section>

        <Section title={`${openState} Pain Rooms`}>
          {open.statePains.length ? <div style={grid}>{open.statePains.map((room) => <RoomCard key={rid(room)} room={room} kind="pain" />)}</div> : <p style={sub}>No active pain rooms in {openState}.</p>}
        </Section>

        <Section title={`${openState} Message Signals`}>
          {open.stateMessages.length ? (
            <div style={grid}>
              {open.stateMessages.map((thread) => (
                <Link key={txt(thread.id)} href="/messages" style={thread.unread ? pulseRed : panel}>
                  <div style={eyebrow}>Message</div>
                  <h2 style={h2}>{txt(thread.subject, "Message Thread")}</h2>
                  <p style={muted}>{txt(thread.lane, "general")} • {thread.unread ? "Unread" : "Seen"}</p>
                </Link>
              ))}
            </div>
          ) : <p style={sub}>No message signals tied to {openState}.</p>}
        </Section>
      </div>
    </main>
  );
