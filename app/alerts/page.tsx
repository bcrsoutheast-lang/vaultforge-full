...
function normalizeRows(data: any) {
  ...
  rows.forEach((row: AlertItem, index: number) => {
    byId.set(alertId(row, index), row);
  });
...
const page: React.CSSProperties = {
  minHeight: "100vh",
  background:
    "radial-gradient(circle at top left, rgba(232,196,107,.13), transparent 30%), radial-gradient(circle at 88% 8%, rgba(248,113,113,.12), transparent 26%), linear-gradient(180deg,#020617,#071326 55%,#020617)",
  color: "white",
...
function AlertCard({
  ...
  return (
    ...
    >
      ...
      <div style={labelStyle}>{shouldPulse ? "New Alert" : status.saved ? "Saved Alert" : status.archived ? "Archived Alert" : status.hidden ? "Hidden Alert" : "Viewed Alert"}</div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
        <span style={{ border: "1px solid rgba(248,113,113,.24)", borderRadius: 999, padding: "8px 11px", color: "#fecaca", background: "rgba(248,113,113,.07)", fontWeight: 900 }}>
          {typeOf(row)}
        </span>
        <span style={{ border: "1px solid rgba(232,196,107,.24)", borderRadius: 999, padding: "8px 11px", color: "#f8e7b0", background: "rgba(232,196,107,.06)", fontWeight: 900 }}>
          {scoreOf(row)}% fit
        </span>
        {!status.seen ? <span style={{ border: "1px solid rgba(57,255,20,.24)", borderRadius: 999, padding: "8px 11px", color: "#39ff14", background: "rgba(57,255,20,.06)", fontWeight: 900 }}>unread</span> : null}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 16 }}>
        <Link href={href} onClick={() => onAction(id, "seen")} style={button}>
          Open Room
        </Link>

        <button type="button" onClick={() => onAction(id, "seen")} style={ghost}>Mark Seen</button>
        <button type="button" onClick={() => onAction(id, "saved")} style={ghost}>{status.saved ? "Unsave" : "Save"}</button>
        <button type="button" onClick={() => onAction(id, "archived")} style={ghost}>{status.archived ? "Unarchive" : "Archive"}</button>
        <button type="button" onClick={() => onAction(id, "hidden")} style={danger}>{status.hidden ? "Unhide" : "Hide"}</button>
      </div>
    </article>
  );
}
...
export default function AlertsPage() {
  ...
  useEffect(() => {
    ...
    setLane(clean(params.get("lane")) || clean(params.get("folder")) || "new");
  }, []);
  ...
  const laneCounts = useMemo(() => {
    ...
    alerts.forEach((row, index) => {
      ...
      if (!st.seen && !st.archived && !st.hidden) counts.new += 1;
      if (!st.archived && !st.hidden) counts[type] = (counts[type] || 0) + 1;
      if (st.saved) counts.saved += 1;
      if (st.archived) counts.archived += 1;
      if (st.hidden) counts.hidden += 1;
    });
  ...
  const filtered = useMemo(() => {
    return alerts.filter((row, index) => {
      ...
      if (lane === "new") return !st.seen && !st.archived && !st.hidden;
      if (lane === "saved") return st.saved;
      if (lane === "archived") return st.archived;
      if (lane === "hidden") return st.hidden;
      return type === lane && !st.archived && !st.hidden;
    });
  ...
  return (
    <main style={page}>
      ...
      <div style={wrap}>
        <section style={panel}>
          <div style={labelStyle}>VaultForge Alert Room</div>
          ...
          <p style={{ color: "#cbd5e1", lineHeight: 1.65, fontSize: 20, marginTop: 0 }}>
            Dedicated alert room. This page does not bounce back to Dashboard. Alerts pulse until opened, marked seen, saved, archived, or hidden.
          </p>
          ...
          >
            <div style={{ color: "#39ff14", fontWeight: 950, letterSpacing: ".12em", textTransform: "uppercase", fontSize: 12, marginBottom: 8 }}>
              Routing Intelligence Notice
            </div>
            <p style={{ margin: 0, color: "#d9ffd0", lineHeight: 1.65, fontSize: 15, fontWeight: 700 }}>
              VaultForge may route the same signal into multiple operational lanes when the opportunity matches several execution categories, pressure conditions, routing triggers, or member fit profiles. This intentional overlap helps ensure high-value opportunities are not missed across active workflows.
            </p>
          </section>

          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 16 }}>
            <Link href="/dashboard" style={button}>Back To Command</Link>
            <button type="button" onClick={markAllSeen} style={ghost}>Mark All Seen</button>
        ...
        <section style={panel}>
          <div style={labelStyle}>Alert Folders</div>
        ...
        {!status && !filtered.length ? (
          <section style={panel}>
            ...
            <p style={{ color: "#cbd5e1", lineHeight: 1.55, margin: "8px 0 0" }}>
              No alerts are currently in {laneTitle(lane)}.
            </p>
          </section>
        ) : null}
