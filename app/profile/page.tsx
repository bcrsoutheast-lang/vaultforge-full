...
// FIXED: This version respects empty arrays and won't reset to defaults
...
function saveProfile(profile: MemberProfile) {
  ...
  console.log("SAVING PROFILE:", profile); // DEBUG

  const now = new Date().toISOString();

  const textOnlyProfile = normalizeProfile({
    ...profile,
    profilePhoto: "",
    photoUrl: "",
    avatar: "",
  ...
  console.log("NORMALIZED FOR SAVE:", textOnlyProfile); // DEBUG

  let saved = true;
  for (const key of PROFILE_KEYS) {
    saved = writeJson(key, textOnlyProfile) && saved;
  }

  const directory = readDirectory().filter((member) => member.id !== textOnlyProfile.id && member.email !== textOnlyProfile.email);
  const slimDirectory = directory.map((member) => ({ ...member, profilePhoto: "", companyLogo: "" }));

  const photoExists = !!readImageBackup(PROFILE_PHOTO_BACKUP_KEY);
  const logoExists = !!readImageBackup(COMPANY_LOGO_BACKUP_KEY);

  const slimForDirectory = {
    ...textOnlyProfile,
    profilePhoto: photoExists ? "__vaultforge_member_profile_photo_v1__" : "",
    companyLogo: logoExists ? "__vaultforge_member_company_logo_v1__" : "",
  };

  saved = writeJson(DIRECTORY_KEY, [slimForDirectory, ...slimDirectory]) && saved;
  console.log("SAVED TO DIRECTORY:", slimForDirectory); // DEBUG
  ...
  return { ok: saved, message: saved ? "Profile saved." : "Save failed. Storage might be full." };
}

async function compressImage(file: File, maxWidth = 300, quality = 0.25): Promise<string> {
...
export default function ProfilePage() {
  ...
  function toggle(key: keyof MemberProfile, value: string) {
    setProfile(prev => {
      const current = new Set(list(prev[key]));
      current.has(value)? current.delete(value) : current.add(value);
      const updated = {...prev, [key]: Array.from(current) };
      console.log("TOGGLED", key, "NEW VALUE:", updated[key]); // DEBUG
      return updated;
    });
  }
  function setBasedCity(city: string) { setProfile(prev => ({...prev, basedCity: city, basedCounty: countyFromCity(city) || prev.basedCounty })); }
  ...
  async function setImage(key: "profilePhoto" | "companyLogo", files: FileList | null) {
    ...
    setProfile(prev => ({...prev, [key]: image, updatedAt: new Date().toISOString() }));
  ...
  function save() {
    setBanner(""); setError(""); 
    setProfile(current => {
      console.log("SAVE CLICKED - CURRENT STATE:", current); // DEBUG
      if (!txt(current.name)) { 
        setError("Add member name before saving."); 
        return current; 
      }
      const result = saveProfile(current); 
      if (!result.ok) { 
        setError(result.message); 
        return current; 
      }
      routeProfileToAdmin(current); 
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <Section title="States Served">
          <ChipSet title="States Operated / Served" options={STATES} selected={profile.statesOperated} onToggle={(value) => toggle("statesOperated"FL", "TN", "AL", "GA", "NC", "SC", "TX"]} selected={profile.statesOperated} onToggle={(value) => toggle("statesOperated", value)} />
          <div style={{ height: 18 }} />
