...
export default function ProfilePage() {
  ...
  return <main style={page}><div style={wrap}><Nav active="profile" />
    ...
    <Section title="Buy Box + Strategy"><ChipSet label="Asset Classes" options={ASSETS} selected={list(profile.assetClasses)} onToggle={(value) => toggle("assetClasses", value)} /><div style={{ height: 18 }} /><ChipSet label="Strategies" options={STRATEGIES} selected={list(profile.strategies)} onToggle={(value) => toggle("strategies", value)} /><div style={{ height: 18 }} /><ChipSet label="Specialties" options={SPECIALTIES} selected={list(profile.specialties)} onToggle={(value) => toggle("specialties", value)} /></Section>
    <Section title="Network Routing"><ChipSet label="Needs" options={NEEDS} selected={list(profile.needs)} onToggle={(value) => toggle("needs", value)} /><div style={{ height: 18 }} /><ChipSet label="Can Provide" options={CAN_PROVIDE} selected={list(profile.canProvide)} onToggle={(value) => toggle("canProvide", value)} /></Section>
    <Section title="Capital + Contact"><div style={grid}><SelectField label="Capital Position" value={txt(profile.capitalPosition, "Unknown")} onChange={(value) => up("capitalPosition", value)} options={CAPITAL_POSITIONS} /><SelectField label="Proof / POF Available" value={txt(profile.proofOfFunds, "Unknown")} onChange={(value) => up("proofOfFunds", value)} options={YESNO} /><SelectField label="Funding Range" value={txt(profile.fundingRange, "Unknown")} onChange={(value) => up("fundingRange", value)} options={FUNDING_RANGES} /><SelectField label="Direct Contact Allowed" value={txt(profile.directContact, "Unknown")} onChange={(value) => up("directContact", value)} options={YESNO} /></div></Section>
    <Section title="AI Bio / Routing Notes"><TextArea label="Bio / Routing Notes" value={txt(profile.bio)} onChange={(value) => up("bio", value)} /></Section>
    <Section title="Save"><button type="button" style={goldBtn} onClick={save}>Save Profile</button>{" "}<Link href="/network" style={btn}>Open Network</Link></Section>
  </div></main>;
}
