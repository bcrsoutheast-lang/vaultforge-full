...
type MemberProfile = {
  id: string; name: string; company: string; email: string; phone: string; title: string;
  memberType: string; basedState: string; basedCity: string; basedCounty: string;
  statesOperated: string[]; countiesServed: string[]; assetClasses: string[];
  strategies: string[]; specialties: string[]; canProvide: string[]; needs: string[];
  fundingRange: string; capitalPosition: string; buyBox: string[]; priceRange: string;
  dealSize: string; responseSpeed: string; verifiedStatus: string; contactPreference: string;
  bio: string; profilePhoto: string; companyLogo: string; createdAt: string; updatedAt: string;
};
...
function txt(value: unknown, fallback = "") { const clean = String(value || "").trim(); return clean || fallback; }
function list(value: unknown): string[] {
  if (Array.isArray(value)) return value.map((x) => String(x).trim()).filter(Boolean);
  if (typeof value === "string" && value.trim()) return value.split(",").map((x) => x.trim()).filter(Boolean);
  return [];
}
function j<T>(raw: string | null, fallback: T): T { try { return raw ? (JSON.parse(raw) as T) : fallback; } catch { return fallback; } }
...
function readImageBackup(key: string) {
  if (!ok()) return ""; const raw = localStorage.getItem(key); if (!raw) return "";
  const parsed = j<string | null>(raw, null); return txt(parsed || raw).replace(/^"|"$/g, "");
}
function writeImageBackup(key: string, value: string) {
  if (!ok()) return false; try { if (value) localStorage.setItem(key, value); else localStorage.removeItem(key); return true; } catch { return false; }
}
...
function defaultProfile(): MemberProfile {
  ...
  return {
    id: "local_member", name: "", company: "", email: "", phone: "", title: "", memberType: "Investor",
    basedState: "GA", basedCity: "", basedCounty: "", statesOperated: ["GA"], countiesServed: [],
    assetClasses: ["Residential"], strategies: ["Wholesale"], specialties: [], canProvide: ["Buying"], needs: ["Deals"],
    fundingRange: "Not listed", capitalPosition: "Not listed", buyBox: [], priceRange: "", dealSize: "Not listed",
    responseSpeed: "24 Hours", verifiedStatus: "Unverified", contactPreference: "VaultForge Message", bio: "",
    profilePhoto: "", companyLogo: "", createdAt: now, updatedAt: now,
  };
}
...
function normalizeProfile(row: any): MemberProfile {
  ...
  return {
    ...
    contactPreference: txt(row?.contactPreference, base.contactPreference), bio: txt(row?.bio || row?.about, base.bio),
    ...
    createdAt: txt(row?.createdAt, base.createdAt), updatedAt: txt(row?.updatedAt, base.updatedAt),
  };
}

function readProfile() {
  if (!ok()) return defaultProfile(); const backupPhoto = readImageBackup(PROFILE_PHOTO_BACKUP_KEY);
  ...
  for (const key of PROFILE_KEYS) {
    ...
    if (found && typeof found === "object") {
      return normalizeProfile({
        ...found, profilePhoto: txt(found.profilePhoto || found.photoUrl || found.avatar || backupPhoto),
        companyLogo: txt(found.companyLogo || found.logoUrl || backupLogo),
      });
    }
  }
  return normalizeProfile({ ...defaultProfile(), profilePhoto: backupPhoto, companyLogo: backupLogo });
}
...
function routeProfileToAdmin(profile: MemberProfile) {
  if (!ok()) return; const queue = j<any[]>(localStorage.getItem(ADMIN_PROFILE_QUEUE_KEY), []);
  const email = txt(profile.email).toLowerCase();
  const row = {
    id: `member-profile-${email || profile.id || Date.now()}`, type: "member", status: "pending_admin_approval",
    title: `${profile.name || "Member"} Profile Approval`, email, name: profile.name, company: profile.company,
    memberType: profile.memberType, profile, createdAt: profile.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
  const next = [row, ...queue.filter((item) => String(item?.id || "") !== row.id && String(item?.email || "").toLowerCase() !== email)];
  writeJson(ADMIN_PROFILE_QUEUE_KEY, next); const approvals = j<Record<string, any>>(localStorage.getItem(MOCK_APPROVALS_KEY), {});
  approvals[`member:${email}`] = {
    ...(approvals[`member:${email}`] || {}), email, kind: "member", submitted: true,
    approved: Boolean(approvals[`member:${email}`]?.approved), adminApproved: Boolean(approvals[`member:${email}`]?.adminApproved),
    paymentStatus: approvals[`member:${email}`]?.paymentStatus || "unpaid",
    accessStatus: approvals[`member:${email}`]?.accessStatus || "pending_admin_approval", updatedAt: new Date().toISOString(),
  };
  writeJson(MOCK_APPROVALS_KEY, approvals); window.dispatchEvent(new Event("vaultforge-admin-profile-queue-change"));
  window.dispatchEvent(new Event("vaultforge-mock-access-change"));
}

function saveProfile(profile: MemberProfile) {
  if (!ok()) return { ok: false, message: "Browser storage unavailable." }; const now = new Date().toISOString();
  ...
  if (photo) writeImageBackup(PROFILE_PHOTO_BACKUP_KEY, photo); if (logo) writeImageBackup(COMPANY_LOGO_BACKUP_KEY, logo);
  const next = normalizeProfile({
    ...profile, profilePhoto: photo, photoUrl: photo, avatar: photo, companyLogo: logo, logoUrl: logo,
    updatedAt: now, createdAt: profile.createdAt || now,
  });
  const directory = readDirectory().filter((member) => member.id !== next.id && member.email !== next.email);
  const slimDirectory = directory.map((member) => ({ ...member, profilePhoto: "", companyLogo: "" }));
  const slimForDirectory = {
    ...next, profilePhoto: photo ? "__vaultforge_member_profile_photo_v1__" : "",
    companyLogo: logo ? "__vaultforge_member_company_logo_v1__" : "",
  };
  let saved = true; for (const key of PROFILE_KEYS) saved = writeJson(key, next) && saved;
...
async function compressImage(file: File, maxWidth = 360, quality = 0.28): Promise<string> {
  return new Promise((resolve) => {
    const reader = new FileReader(); reader.onerror = () => resolve(""); reader.onload = () => {
      const img = new Image(); img.onerror = () => resolve(""); img.onload = () => {
        try {
          const scale = Math.min(1, maxWidth / img.width); const canvas = document.createElement("canvas");
...
function stopKeys(event: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) { event.stopPropagation(); }
...
export default function ProfilePage() {
  ...
  const completion = useMemo(() => {
    let score = 0; if (profile.name) score += 10; if (profile.email || profile.phone) score += 10;
    if (profile.basedCity && profile.basedState) score += 10; if (profile.statesOperated.length) score += 10;
    if (profile.assetClasses.length) score += 10; if (profile.strategies.length) score += 10;
    if (profile.canProvide.length) score += 12; if (profile.needs.length) score += 8;
    if (profile.fundingRange !== "Not listed") score += 10; if (profile.bio) score += 10;
    return Math.min(100, score);
  }, [profile]);

  function update(key: keyof MemberProfile, value: any) { setProfile({ ...profile, [key]: value }); }
  function toggle(key: keyof MemberProfile, value: string) {
    const current = new Set(list(profile[key])); current.has(value) ? current.delete(value) : current.add(value);
    update(key, Array.from(current));
  }
  function setBasedCity(city: string) { setProfile({ ...profile, basedCity: city, basedCounty: countyFromCity(city) || profile.basedCounty }); }

  async function setImage(key: "profilePhoto" | "companyLogo", files: FileList | null) {
    const file = Array.from(files || [])[0]; if (!file) return; setError(""); setBanner("Compressing and saving image...");
    const image = await compressImage(file);
    if (!image) { setError("Image could not be loaded. Try a smaller JPG/PNG."); setBanner(""); return; }
    ...
    const next = { ...profile, [key]: image, updatedAt: new Date().toISOString() } as MemberProfile; setProfile(next);
    for (const storageKey of PROFILE_KEYS) {
      const current = j<any | null>(localStorage.getItem(storageKey), null);
      if (current && typeof current === "object") {
        writeJson(storageKey, {
          ...current, [key]: image, photoUrl: key === "profilePhoto" ? image : current.photoUrl,
          avatar: key === "profilePhoto" ? image : current.avatar, logoUrl: key === "companyLogo" ? image : current.logoUrl,
          updatedAt: next.updatedAt,
        });
      }
    }
    if (!backupSaved) { setError("Image preview loaded, but browser storage is full. Delete old saved/archived items or clear site data before final launch."); setBanner(""); return; }
    setBanner(key === "profilePhoto" ? "Profile photo saved." : "Company logo saved.");
  }

  function save() {
    setBanner(""); setError(""); if (!txt(profile.name)) { setError("Add member name before saving."); return; }
    const result = saveProfile(profile); if (!result.ok) { setError(result.message); return; }
    routeProfileToAdmin(profile); const message = "Profile saved and routed to admin for approval.";
    setBanner(message); window.alert(message); window.scrollTo({ top: 0, behavior: "smooth" });
  }
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <Section title="Profile Preview">
          <div style={grid}>
            <div style={activePanel}>
              ...
              <h2 style={h2}>{txt(profile.name) || "Owner"}</h2>
              <p style={sub}>{txt(profile.company)}</p>
        ...
        <Section title="Photos">
          ...
          <div style={grid}>
            <div style={panel}>
              ...
              <div style={{ ...row, marginTop: 12 }}>
                <button type="button" style={redBtn} onClick={() => {
                  writeImageBackup(PROFILE_PHOTO_BACKUP_KEY, ""); update("profilePhoto", "");
                  for (const storageKey of PROFILE_KEYS) {
                    const current = j<any | null>(localStorage.getItem(storageKey), null);
            ...
            <div style={panel}>
              ...
              <div style={{ ...row, marginTop: 12 }}>
                <button type="button" style={redBtn} onClick={() => {
                  writeImageBackup(COMPANY_LOGO_BACKUP_KEY, ""); update("companyLogo", "");
                  for (const storageKey of PROFILE_KEYS) {
                    const current = j<any | null>(localStorage.getItem(storageKey), null);
        ...
        <Section title="Asset + Strategy Fit">
          ...
          <div style={{ height: 18 }} />
          <ChipSet title="Strategies" options={STRATEGIES} selected={profile.strategies} onToggle={(value) => toggle("strategies", value)} /> <div style={{ height: 18 }} />
        ...
        <Section title="Capital + Buy Box">
          ...
          <div style={{ height: 18 }} />
          <TextArea title="Buy Box" value={profile.buyBox.join(", ")} onChange={(value) => update("buyBox", list(value))} />
        </Section>

        <Section title="About">
          <TextArea title="Bio" value={profile.bio} onChange={(value) => update("bio", value)} />
        </Section>
