...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (!ok()) return;
    const found = PROFILE_KEYS.map((key) => j<Profile | null>(localStorage.getItem(key), null)).find((row) => row && typeof row === "object");
  ...
  async function photo(files: FileList | null) {
    ...
    if (imgs[0]) up("profilePhoto", imgs[0]);
  }
  function save() {
    if (!ok()) return;
    PROFILE_KEYS.forEach((key) => saveSafe(key, profile));
    setSaved("Profile saved.");
    window.dispatchEvent(new Event("vaultforge-profile-change"));
  }

  return <main style={page}><div style={wrap}><Nav active="profile" />
    <section style={hero}><div style={eyebrow}>Profile</div><h1 style={h1}>AI matching data.</h1><p style={sub}>Profile restored with photo upload, one clean state row, counties, markets, routing, contact preferences, and network data.</p></section>
    ...
    <Section title="Profile Photo">
      ...
      <input type="file" accept="image/*" onChange={(event) => photo(event.target.files)} />
      <p style={muted}>Photo saves locally for now.</p>
    </Section>

    <Section title="States Operated In">
      <ChipSet label="States" options={STATES} selected={list(profile.statesOperated)} onToggle={(value) => toggle("statesOperated", value)} />
    </Section>

    <Section title="County Popup Selector">
      <ChipSet label="Counties" options={COUNTIES} selected={list(profile.counties)} onToggle={(value) => toggle("counties", value)} />
    </Section>

    <Section title="Markets / Submarkets">
      <ChipSet label="Markets" options={MARKETS} selected={list(profile.markets)} onToggle={(value) => toggle("markets", value)} />
    </Section>
    ...
    <Section title="Buy Box + Strategy">
      ...
      <div style={{ height: 18 }} />
      <ChipSet label="Specialties" options={["Distress", "Funding Gap", "Off Market", "Construction", "Land", "Commercial", "Residential", "Creative Finance", "Insurance", "Permits"]} selected={list(profile.specialties)} onToggle={(value) => toggle("specialties", value)} />
    </Section>
    ...
    <Section title="Network Routing">
      ...
      <div style={{ height: 18 }} />
      <ChipSet label="Can Provide" options={["Capital", "Buying", "Lending", "Contractors", "Legal", "Insurance", "Property Management", "Development", "Operations", "Introductions"]} selected={list(profile.canProvide)} onToggle={(value) => toggle("canProvide", value)} />
    </Section>

    <Section title="Capital + Contact">
      <div style={grid}>
        <SelectField label="Capital Position" value={txt(profile.capitalPosition, "Unknown")} onChange={(value) => up("capitalPosition", value)} options={["Unknown", "Cash Buyer", "Private Capital", "Hard Money", "Bank Lending", "JV Capital", "Needs Capital"]} />
        <SelectField label="Proof Of Funds" value={txt(profile.proofOfFunds, "Unknown")} onChange={(value) => up("proofOfFunds", value)} options={YESNO} />
