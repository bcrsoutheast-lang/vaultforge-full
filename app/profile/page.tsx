...
export default function ProfilePage(){
 const [p,setP]=useState<Profile>({}); const [saved,setSaved]=useState(false); useEffect(()=>setP(readProfile()),[]); const st=profileStates(p);
 function toggle(s:string){ const set=new Set(st); set.has(s)?set.delete(s):set.add(s); setP({...p,statesOperatedIn:Array.from(set)}); }
 function save(){ if(!ok())return; const next={...p,id:p.id||txt(p.email,"current-profile"),statesOperatedIn:st,updatedAt:new Date().toISOString()}; localStorage.setItem(PROFILE_KEY,JSON.stringify(next)); const rows=members().filter(m=>txt(m.id||m.email||m.name||m.company).toLowerCase()!==txt(next.id||next.email||next.name||next.company).toLowerCase()); localStorage.setItem(NETWORK_KEY,JSON.stringify([next,...rows])); setP(next); setSaved(true); setTimeout(()=>setSaved(false),2200); window.dispatchEvent(new Event("vaultforge-network-change")); }
 return <main style={page}><div style={wrap}><Nav active="profile"/>{saved?<section style={{...card,borderColor:"rgba(255,220,104,.8)"}}><div style={eyebrow}>Saved</div><p style={sub}>Profile saved and connected to Network.</p></section>:null}<section style={hero}><div style={eyebrow}>Profile</div><h1 style={h1}>AI matching data.</h1><p style={sub}>One clean state row. These states control routing, networking, and member contact lanes.</p></section><section style={card}><div style={eyebrow}>Identity</div><div style={grid}><Field l="Name" v={txt(p.name)} c={v=>setP({...p,name:v})}/><Field l="Company" v={txt(p.company)} c={v=>setP({...p,company:v})}/><Field l="Email" v={txt(p.email)} c={v=>setP({...p,email:v})}/><Field l="Phone" v={txt(p.phone)} c={v=>setP({...p,phone:v})}/><Field l="Member Type" v={txt(p.memberType)} c={v=>setP({...p,memberType:v})}/><Field l="Photo URL" v={txt(p.photoUrl)} c={v=>setP({...p,photoUrl:v})}/></div></section><section style={card}><div style={eyebrow}>Geography Intelligence</div><h2 style={h2}>States operated in.</h2><p style={{...sub,marginBottom:18}}>This single row feeds routing, network search, and state project boards.</p><div style={row}>{STATES.map(s=><button key={s} type="button" onClick={()=>toggle(s)} style={st.includes(s)?chipActive:chip}>{s}</button>)}</div></section><section style={card}><div style={eyebrow}>Buy Box + Strategy</div><div style={grid}><Field l="Markets / Submarkets" v={list(p.markets).join(", ")} c={v=>setP({...p,markets:v})}/><Field l="Strategies" v={list(p.strategies).join(", ")} c={v=>setP({...p,strategies:v})}/><Field l="Capital / Capacity" v={txt(p.capital)} c={v=>setP({...p,capital:v})}/><Field l="Notes" v={txt(p.notes)} c={v=>setP({...p,notes:v})}/></div></section><section style={card}><button onClick={save} style={goldBtn}>Save Profile</button><Link href="/network" style={{...btn,marginLeft:10}}>Open Network</Link></section></div></main>
}
function Field({l,v,c}:{l:string;v:string;c:(v:string)=>void}){return <label style={{display:"block"}}><div style={label}>{l}</div><input style={input} value={v} onChange={e=>c(e.target.value)}/></label>}
