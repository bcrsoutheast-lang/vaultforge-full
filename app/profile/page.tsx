...
function normalizeProfile(row: any): MemberProfile {
  ...
  return {
    ...
    company: txt(row?.company || row?.companyName, base.company), email,
    phone: txt(row?.phone || row?.phoneNumber, base.phone), title: txt(row?.title || row?.roleTitle, base.title),
...
function saveProfile(profile: MemberProfile) {
  ...
  let saved = true;
  for (const key of PROFILE_KEYS) saved = writeJson(key, next) && saved;
