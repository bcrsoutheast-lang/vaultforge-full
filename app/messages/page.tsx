...
const LANES: {
  ...
  { key: "alerts", title: "Alerts", label: "ALERTS", description: "Alert follow-up, owner requests, and urgent message traffic.", href: "/alerts" },
  { key: "pain", title: "Pain", label: "PAIN", description: "Pain signal conversations and problem-routing follow-up.", href: "/pain-feed" },
  { key: "signals", title: "Signals", label: "SIGNALS", description: "Signal-room messages and intelligence follow-up.", href: "/signals" },
  { key: "routing", title: "Routing", label: "ROUTING", description: "Routing requests, member-fit paths, and execution handoffs.", href: "/routing-inbox" },
  { key: "introductions", title: "Introductions", label: "INTRO", description: "Controlled introduction conversations.", href: "/introductions" },
  { key: "projects", title: "Projects", label: "PROJECTS", description: "Project and deal-room communication.", href: "/projects" },
  { key: "members", title: "Members", label: "MEMBERS", description: "Member-to-member and network messages.", href: "/members" },
  { key: "general", title: "General", label: "GENERAL", description: "General messages that are not tied to a lane yet.", href: "/messages" },
];
...
function isHidden(row: MessageRow) {
  const status = lower(row.status);
  return row.is_deleted === true || row.deleted === true || row.is_archived === true || row.archived === true || status === "deleted" || status === "archived";
}
...
export default function MessagesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setRows(nextRows.filter((row: MessageRow) => !isHidden(row)));
  ...
  const conversations = useMemo(() => {
    ...
    rows.filter((row) => !isHidden(row)).forEach((row) => {
  ...
  const visibleConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      if (selectedLane !== "all" && conversation.lane !== selectedLane) return false;
      if (!q) return true;
  ...
  async function cleanupConversation(conversation: Conversation, action: "archive" | "delete") {
    const email = viewer || currentEmail();
    setBusyKey(`${action}:${conversation.key}`);
    ...
    try {
      ...
      setRows((current) =>
        current.filter((row) => conversationKey(row) !== conversation.key)
      );
      ...
      <div style={wrap}>
        ...
        <section className="vf-two-col" style={heroGrid}>
          ...
          <aside style={tape}>
            ...
            {LANES.slice(0, 6).map((lane) => (
              <button key={lane.key} type="button" onClick={() => setSelectedLane(lane.key)} style={tapeRow}>
        ...
        <section className="vf-two-col" style={laneGrid}>
          {LANES.map((lane) => {
            ...
            return (
              <button key={lane.key} type="button" onClick={() => setSelectedLane(lane.key)} style={active ? laneCardActive : laneCard}>
        ...
        <section style={filterPanel}>
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search selected messages..." style={input} />
        </section>
        ...
        <section style={{ display: "grid", gap: 18 }}>
          {visibleConversations.map((conversation) => {
            ...
            const isArchiving = busyKey === `archive:${conversation.key}`;
            const isDeleting = busyKey === `delete:${conversation.key}`;
            ...
            return (
              <article key={conversation.key} style={conversationCard}>
                ...
                <p style={messagePreview}>{conversation.latestMessage || "No preview available."}</p>
                ...
                <div className="vf-actions" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 18 }}>
                  ...
                  <button type="button" onClick={() => cleanupConversation(conversation, "archive")} disabled={!!busyKey} style={ghostButton}>
                    {isArchiving ? "Archiving..." : "Archive"}
                  </button>

                  <button type="button" onClick={() => cleanupConversation(conversation, "delete")} disabled={!!busyKey} style={dangerButton}>
...
const heroGrid: React.CSSProperties = { display: "grid", gridTemplateColumns: "minmax(0,1.55fr) minmax(280px,.8fr)", gap: 22, border: "1px solid rgba(232,196,107,.18)", borderRadius: 30, padding: 28, marginBottom: 22, background: "linear-gradient(145deg,rgba(255,255,255,.06),rgba(255,255,255,.025))", boxShadow: "0 28px 86px rgba(0,0,0,.30)" };
const eyebrow: React.CSSProperties = { color: "#e8c46b", fontWeight: 950, letterSpacing: ".18em", textTransform: "uppercase", fontSize: 12 };
const heroTitle: React.CSSProperties = { fontSize: "clamp(54px,10vw,104px)", lineHeight: .88, letterSpacing: "-.075em", margin: "12px 0 22px" };
const lead: React.CSSProperties = { color: "#cbd5e1", fontSize: 20, lineHeight: 1.55, maxWidth: 760 };
const tape: React.CSSProperties = { border: "1px solid rgba(255,255,255,.12)", borderRadius: 26, padding: 20, background: "rgba(0,0,0,.18)" };
const tapeTitle: React.CSSProperties = { color: "#e8c46b", letterSpacing: ".16em", textTransform: "uppercase", fontWeight: 950, fontSize: 12, marginBottom: 12 };
const tapeRow: React.CSSProperties = { width: "100%", display: "flex", justifyContent: "space-between", gap: 12, padding: "13px 0", color: "white", background: "transparent", border: 0, borderBottom: "1px solid rgba(255,255,255,.09)", textAlign: "left", fontWeight: 850, cursor: "pointer" };
const sectionHeader: React.CSSProperties = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap", margin: "26px 0 14px" };
...
const laneSmall: React.CSSProperties = { color: "#38bdf8", letterSpacing: ".16em", textTransform: "uppercase", fontWeight: 950, fontSize: 12 };
...
const laneDescription: React.CSSProperties = { color: "#cbd5e1", lineHeight: 1.5, margin: 0, maxWidth: 520 };
const filterPanel: React.CSSProperties = { border: "1px solid rgba(232,196,107,.18)", borderRadius: 28, padding: 22, marginBottom: 18, background: "rgba(255,255,255,.035)" };
const input: React.CSSProperties = { width: "100%", marginTop: 18, boxSizing: "border-box", borderRadius: 18, border: "1px solid rgba(255,255,255,.14)", background: "#081224", color: "white", padding: 16, fontSize: 16, outline: "none" };
const conversationCard: React.CSSProperties = { position: "relative", border: "1px solid rgba(255,255,255,.12)", borderRadius: 28, padding: 24, background: "rgba(255,255,255,.035)" };
...
const messagePreview: React.CSSProperties = { color: "#dbeafe", fontSize: 21, lineHeight: 1.5, marginTop: 18 };
