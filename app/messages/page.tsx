...
export default function MessagesPage() {
  ...
  const [attachedRoomOwner, setAttachedRoomOwner] = useState('')
  ...
  // Pull recipient from URL or attached room data
  useEffect(() => {
    const urlRecipient = searchParams.get('recipient')
    const urlRoom = searchParams.get('room')
    const urlTitle = searchParams.get('title')
    
    // If we have a room attached, try to get owner from localStorage
    let ownerEmail = urlRecipient || ''
    if (urlRoom) {
      const raw = localStorage.getItem('vf_rooms')
      if (raw) {
        try {
          const rooms = JSON.parse(raw)
          const room = rooms.find((r: any) => r.title === urlRoom)
          if (room?.owner_email) ownerEmail = room.owner_email
        } catch {}
      }
    }
    
    setAttachedRoomOwner(ownerEmail)
  ...
  async function handleSendMessage() {
    ...
    if (!form.recipient) return alert('Owner email missing for this deal')
