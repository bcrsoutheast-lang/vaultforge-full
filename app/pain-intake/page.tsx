...
export default function PainIntakePage() {
  ...
  function save() {
    ...
    setSavedId(id);
    setBanner("Pain room saved. Open Room will now go to the real saved pain room.");
  }
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <section style={hero}>
          ...
          <h1 style={h1}>Problem execution room.</h1>
          <p style={sub}>Capture the real pressure: blocker, risk, control, capital, timeline, solver type, photos, contact, and AI context.</p>
        </section>
        ...
        <Section title="Live Preview">
          <div style={activePanel}>
            ...
            <p style={muted}>AI: {intelligence.banner} • Severity {intelligence.severityScore}% • Difficulty {intelligence.difficulty}%</p>
          </div>
        </Section>

        <Section title="Problem Type">
          <ChipSet title="Pain Type" options={PAIN_TYPES} selected={form.painTypes} onToggle={(value) => toggle("painTypes", value)} />
        </Section>

        <Section title="Solution Routing">
          <ChipSet title="Needs" options={NEEDS} selected={form.needs} onToggle={(value) => toggle("needs", value)} />
        </Section>
        ...
        <Section title="Severity + Pressure">
          ...
          <div style={{ height: 18 }} />
          <ChipSet title="Current Blockers" options={BLOCKERS} selected={form.blockers} onToggle={(value) => toggle("blockers", value)} />
          <div style={{ height: 18 }} />
        ...
        <Section title="Outcome Logic">
          <div style={grid}>
            ...
            <Field title="Worst Case Scenario" value={form.worstCase} onChange={(value) => update("worstCase", value)} />
        ...
        <Section title="Photos">
          ...
          {form.photos.length ? <div style={grid}>{form.photos.map((photo, index) => <div key={`${photo.slice(0, 20)}-${index}`} style={panel}><img src={photo} alt={`Pain ${index + 1}`} style={photoStyle} /><button type="button" style={redBtn} onClick={() => removePhoto(index)}>Delete Photo</button></div>)}</div> : null}
        </Section>
