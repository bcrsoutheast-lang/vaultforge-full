...
export default function PainIntakePage() {
  ...
  const propertyTypes = useMemo(() => assetTypes(txt(form.assetClass, "Residential")), [form.assetClass]);

  function up(key: string, value: unknown) {
    setForm({ ...form, [key]: value });
  }

  function toggle(key: "routingNeeds" | "painTypes" | "blockers" | "riskTypes" | "documentsAvailable", value: string) {
  ...
  function setAsset(value: string) {
    const nextTypes = assetTypes(value);
    setForm({ ...form, assetClass: value, propertyType: nextTypes[0] });
  }

  function updateCity(value: string) {
    const county = countyFromCity(value);
    setForm({ ...form, city: value, county: county || form.county || "" });
  }

  function analyzer() {
    return `Pain analyzer: ${list(form.painTypes).join(", ") || "Problem"} in ${loc(form)}. Severity ${txt(form.severity)}. Time pressure ${txt(form.timePressure)}. Capital pressure ${txt(form.capitalPressure)}. Needs ${list(form.routingNeeds).join(", ") || "solver"}. Root cause: ${txt(form.rootCause, "not stated")}. Blockers: ${list(form.blockers).join(", ") || "not selected"}. Next move: verify control, identify the one blocker that stops execution, route to the highest-fit solver, then open a message thread.`;
  }
  ...
  function save() {
    ...
    if (!list(form.painTypes).length) {
      setError("Select at least one pain type.");
      return;
    }

    const cover = photos[0] || "";
    const id = saveRoom("pain", {
      ...form,
    ...
    if (!id) {
      setError("Pain room did not save. Browser storage may be full. Delete extra photos or old test data and try again.");
      return;
    }

    setSavedId(id);
    setBanner("Pain room saved. It is now active in Command, Network, Pain Rooms, Alerts, and solver matching.");
  }

  return (
    <main style={page}>
      <div style={wrap}>
        <Nav active="pain-intake" />

        {banner ? <section style={activePanel}><div style={eyebrow}>Saved</div><h2 style={h2}>{banner}</h2><div style={{ ...row, marginTop: 18 }}><Link href={`/pain-rooms/${encodeURIComponent(savedId)}`} style={goldBtn}>Open Room</Link><button type="button" style={btn} onClick={() => { setSavedId(""); setBanner(""); }}>Create Another</button><Link href="/network" style={btn}>Go Network</Link></div></section> : null}
        ...
        <section style={hero}>
          ...
          <h1 style={h1}>Problem execution form.</h1>
          <p style={sub}>Pain captures pressure, blocker, risk, control, capital, documents, and solver routing.</p>
        </section>

        <Section title="Live Preview">
          <div style={activePanel}>
            {photos[0] ? <img src={photos[0]} alt="Pain preview" style={photoStyle} /> : null}
            <div style={eyebrow}>{txt(form.severity, "High")} • {txt(form.assetClass, "Residential")}</div>
            <h2 style={h2}>{txt(form.title, "Untitled Pain Room")}</h2>
            <p style={sub}>{loc(form)}</p>
            <p style={muted}>Problem: {list(form.painTypes).join(", ") || "None"} • Needs: {list(form.routingNeeds).join(", ") || "None"}</p>
          </div>
        </Section>

        <Section title="Asset Class">
          <ChipSet label="Asset Class" options={ASSETS} selected={[txt(form.assetClass, "Residential")]} onToggle={setAsset} />
        </Section>

        <Section title="Problem Type">
          <ChipSet label="Pain Type" options={PAIN_TYPES} selected={list(form.painTypes)} onToggle={(value) => toggle("painTypes", value)} />
        </Section>

        <Section title="Solution Routing">
          <ChipSet label="Needs" options={NEEDS} selected={list(form.routingNeeds)} onToggle={(value) => toggle("routingNeeds", value)} />
        </Section>

        <Section title="Severity + Pressure">
          <div style={grid}>
            <SelectField label="Severity" value={txt(form.severity, "High")} onChange={(value) => up("severity", value)} options={SEVERITY} />
            <SelectField label="Time Pressure" value={txt(form.timePressure, "7 Days")} onChange={(value) => up("timePressure", value)} options={TIME} />
            <SelectField label="Capital Pressure" value={txt(form.capitalPressure, "Unknown")} onChange={(value) => up("capitalPressure", value)} options={CAPITAL} />
            <SelectField label="Control Status" value={txt(form.controlStatus, "Unknown")} onChange={(value) => up("controlStatus", value)} options={CONTROL} />
            <SelectField label="Best Contact" value={txt(form.bestContact, "VaultForge Message")} onChange={(value) => up("bestContact", value)} options={BEST_CONTACT} />
          </div>
          <div style={{ height: 18 }} />
          <ChipSet label="Current Blockers" options={BLOCKERS} selected={list(form.blockers)} onToggle={(value) => toggle("blockers", value)} />
          <div style={{ height: 18 }} />
          <ChipSet label="Risk Types" options={RISK} selected={list(form.riskTypes)} onToggle={(value) => toggle("riskTypes", value)} />
        </Section>

        <Section title="Property + Market">
          <div style={grid}>
            <Field label="Pain Room Title" value={txt(form.title)} onChange={(value) => up("title", value)} />
            <SelectField label="State" value={txt(form.state, "GA")} onChange={(value) => up("state", value)} options={STATES} />
            <Field label="City" value={txt(form.city)} onChange={updateCity} />
        ...
        <Section title="Control + Status">
          <div style={grid}>
            <Field label="Current Status" value={txt(form.currentStatus)} onChange={(value) => up("currentStatus", value)} />
            <Field label="Owner / Seller Situation" value={txt(form.ownerSituation)} onChange={(value) => up("ownerSituation", value)} />
            <Field label="Access Status" value={txt(form.accessStatus)} onChange={(value) => up("accessStatus", value)} />
            <Field label="Title Status" value={txt(form.titleStatus)} onChange={(value) => up("titleStatus", value)} />
            <Field label="Insurance Status" value={txt(form.insuranceStatus)} onChange={(value) => up("insuranceStatus", value)} />
            <Field label="Permit / City Status" value={txt(form.permitStatus)} onChange={(value) => up("permitStatus", value)} />
            <Field label="Tenant / Occupancy Status" value={txt(form.tenantStatus)} onChange={(value) => up("tenantStatus", value)} />
            <Field label="Legal Status" value={txt(form.legalStatus)} onChange={(value) => up("legalStatus", value)} />
          </div>
        </Section>
        ...
        <Section title="Numbers + Burn">
          <div style={grid}>
            ...
            <Field label="Monthly Burn Rate" value={txt(form.monthlyBurnRate)} onChange={(value) => up("monthlyBurnRate", value)} />
            <Field label="Money Needed Now" value={txt(form.moneyNeededNow)} onChange={(value) => up("moneyNeededNow", value)} />
            <Field label="Deadline / Date Pressure" value={txt(form.deadline)} onChange={(value) => up("deadline", value)} />
          </div>
        </Section>

        <Section title="Outcome Logic">
          <div style={grid}>
            <Field label="Root Cause" value={txt(form.rootCause)} onChange={(value) => up("rootCause", value)} />
            <Field label="Best Realistic Outcome" value={txt(form.bestOutcome)} onChange={(value) => up("bestOutcome", value)} />
            <Field label="Worst Case Scenario" value={txt(form.worstCase)} onChange={(value) => up("worstCase", value)} />
            <Field label="Desired Solution" value={txt(form.desiredSolution)} onChange={(value) => up("desiredSolution", value)} />
          </div>
        </Section>

        <Section title="Asset-Specific Fields">
          <div style={grid}>
            {txt(form.assetClass) === "Commercial" ? (
              <>
                <Field label="NOI / Income" value={txt(form.noi)} onChange={(value) => up("noi", value)} />
                <Field label="Units / Tenants" value={txt(form.units)} onChange={(value) => up("units", value)} />
                <Field label="Cap Rate" value={txt(form.capRate)} onChange={(value) => up("capRate", value)} />
              </>
            ) : txt(form.assetClass) === "Land" ? (
              <>
                ...
                <Field label="Utilities" value={txt(form.utilities)} onChange={(value) => up("utilities", value)} />
                <Field label="Entitlement Status" value={txt(form.entitlementStatus)} onChange={(value) => up("entitlementStatus", value)} />
              </>
            ) : (
              <>
        ...
        <Section title="Documents Available">
          <ChipSet label="Documents" options={DOCUMENTS} selected={list(form.documentsAvailable)} onToggle={(value) => toggle("documentsAvailable", value)} />
        </Section>
        ...
        <Section title="Contact + Notes">
          <div style={grid}>
            ...
            <TextArea label="Problem Notes / AI Context" value={txt(form.notes)} onChange={(value) => up("notes", value)} />
          </div>
        </Section>
        ...
        <Section title="Save">
          <button type="button" style={goldBtn} onClick={save}>Save Pain Room</button>
        </Section>
      </div>
    </main>
  );
