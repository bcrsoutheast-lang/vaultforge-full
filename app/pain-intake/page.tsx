...
export default function PainIntakePage() {
  ...
  function toggle(key: "routingNeeds" | "painTypes" | "blockers" | "riskTypes", value: string) {
  ...
  async function submit() {
    ...
    const id = saveRoom("pain", {
      ...form,
      title: txt(form.title, "Untitled Pain Room"),
      coverPhoto: cover,
      photoUrl: cover,
      imageUrl: cover,
      photos: images,
      photoUrls: images,
      analyzer: `Pain analyzer: ${list(form.painTypes).join(", ") || "Problem"} in ${txt(form.city)}, ${txt(form.county)}, ${txt(form.state)}. Severity ${txt(form.severity)}. Needs ${list(form.routingNeeds).join(", ") || "solver"}.`,
    });
  ...
  return <main style={page}><div style={wrap}><Nav active="pain-intake" />
    <section style={hero}><div style={eyebrow}>Adaptive Pain Intake</div><h1 style={h1}>Problem Room.</h1><p style={sub}>Full pain form restored with extra problem-routing fields.</p></section>
    ...
    <Section title="Severity + Pressure">
      <div style={grid}>
        ...
        <SelectField label="Control Status" value={txt(form.controlStatus, "Unknown")} onChange={(value) => up("controlStatus", value)} options={["Unknown", "Owner Controlled", "Contract Controlled", "Partner Controlled", "Bank Controlled", "Court / Estate", "No Control Yet"]} />
      </div>
      <div style={{ height: 18 }} />
    ...
    <Section title="Outcome Logic">
      <div style={grid}>
        ...
        <Field label="Best Outcome" value={txt(form.bestOutcome)} onChange={(value) => up("bestOutcome", value)} />
        <Field label="Worst Case" value={txt(form.worstCase)} onChange={(value) => up("worstCase", value)} />
    ...
    <Section title="Save"><button style={goldBtn} onClick={submit}>Save Pain Room</button></Section>
  </div></main>;
}
