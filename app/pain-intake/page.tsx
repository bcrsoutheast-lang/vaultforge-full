...
const PAIN_NEEDS = ["Lender", "Operator", "Contractor", "Buyer", "Attorney", "Insurance Adjuster", "City Expeditor", "Private Capital"];
const PAIN_TYPES = ["Funding Gap", "Foreclosure", "Stalled Construction", "Contractor Problem", "Title Problem", "Permit Problem", "City Violation", "Tenant Issue", "Partnership Dispute", "Emergency Exit", "Insurance Claim", "Fire Damage", "Mold", "Structural"];
...
export default function PainIntakePage() {
  ...
  function toggleList(key: "routingNeeds" | "painTypes", value: string) {
  ...
  async function submit() {
    ...
    const id = saveRoom("pain", {
      ...
      photos: images,
      photoUrls: images,
      analyzer: `Pain analyzer: ${list(form.painTypes).join(", ") || "Problem"} in ${txt(form.city)}, ${txt(form.county)}, ${txt(form.state)}. Needs ${list(form.routingNeeds).join(", ") || "solver"}.`,
    });
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <section style={hero}>
          ...
          <h1 style={h1}>Problem Room.</h1>
          <p style={sub}>Stable form. Saves active pain room, cover photo, and opens the new room.</p>
        </section>

        {saving ? (
          <section style={card}>
            <p style={sub}>Saving pain room...</p>
          </section>
        ) : null}
        ...
        <section style={card}>
          <ChipSet label="Pain Type" options={PAIN_TYPES} selected={list(form.painTypes)} onToggle={(value) => toggleList("painTypes", value)} />
        </section>

        <section style={card}>
          <ChipSet label="Needs" options={PAIN_NEEDS} selected={list(form.routingNeeds)} onToggle={(value) => toggleList("routingNeeds", value)} />
        </section>
        ...
        <section style={card}>
          <div style={grid}>
            ...
            <Field label="Time Pressure" value={txt(form.timePressure)} onChange={(value) => up("timePressure", value)} />
          </div>
        </section>
