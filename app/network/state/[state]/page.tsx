...
export default function Page({params,searchParams}:{params:{state:string};searchParams?:{type?:string}}){const state=txt(params.state,"GA").toUpperCase();const [kind,setKind]=useState<RoomKind>(searchParams?.type==="pain"?"pain":"deal");const [tick,setTick]=useState(0);const load=()=>setTick(x=>x+1);useEffect(()=>{load();const ev=["storage","vaultforge-room-state-change","vaultforge-room-read-change"];ev.forEach(e=>window.addEventListener(e,load));return()=>ev.forEach(e=>window.removeEventListener(e,load));},[kind,state]);const dr=roomsByState("deal",state),pr=roomsByState("pain",state),du=unreadByState("deal",state),pu=unreadByState("pain",state),rooms=kind==="deal"?dr:pr;return <main style={page}><style>{`@keyframes vfPulse{0%{box-shadow:0 0 0 0 rgba(255,70,70,.45)}70%{box-shadow:0 0 0 13px rgba(255,70,70,0)}100%{box-shadow:0 0 0 0 rgba(255,70,70,0)}}`}</style><div style={wrap}><Nav active="network"/><section style={hero}><div style={eyebrow}>{state} Project Desk</div><h1 style={h1}>Deal and Pain projects.</h1></section><section style={card}><div style={twoGrid}><button onClick={()=>setKind("deal")} style={{...splitCard,textAlign:"left",cursor:"pointer",...(du.length?redPulse:{})}}><div style={smallEyebrow}>Deal Projects</div><h3 style={h3}>{dr.length} Deal Rooms</h3><p style={muted}>{du.length} unread</p></button><button onClick={()=>setKind("pain")} style={{...splitCard,textAlign:"left",cursor:"pointer",...(pu.length?redPulse:{})}}><div style={smallEyebrow}>Pain Projects</div><h3 style={h3}>{pr.length} Pain Rooms</h3><p style={muted}>{pu.length} unread</p></button></div></section><section style={card}><div style={grid}>{rooms.length?rooms.map(r=><article key={rid(r)} style={{...inner,...(unread(kind,r)?redPulse:{})}}><div style={smallEyebrow}>{kind==="deal"?"Deal Project":"Pain Project"}</div><h3 style={h3}>{titleFor(r,kind)}</h3><p style={muted}>{loc(r)}</p><div style={{...row,marginTop:16}}><Link href={href(kind,r)} style={goldBtn}>Open Room</Link><button onClick={()=>{setRoomState(r,kind,"saved");load();}} style={btn}>Save</button><button onClick={()=>{setRoomState(r,kind,"archived");load();}} style={btn}>Archive</button><button onClick={()=>{setRoomState(r,kind,"deleted");load();}} style={redBtn}>Delete</button></div></article>):<div style={inner}><h3 style={h3}>No active {kind} projects in {state}.</h3></div>}</div></section></div></main>}
