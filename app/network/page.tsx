...
export default function NetworkPage(){
  const [profile,setProfile]=useState<Profile>({statesOperated:["GA"]});
  ...
  const states=list(profile.statesOperated);
  const [openState,setOpenState]=useState(states[0]||"GA");
  return <main style={page}><div style={wrap}><Nav active="network"/>
    <section style={hero}><div style={eyebrow}>Network</div><h1 style={h1}>State member network.</h1><p style={sub}>Click a state to view member profile, contact options, and room fit.</p></section>
    <Section title="Your States"><div style={row}>{(states.length?states:STATES).map(s=><button key={s} style={openState===s?goldBtn:btn} onClick={()=>setOpenState(s)}>{s}</button>)}</div></Section>
    <Section title={`${openState} Member Profiles`}><div style={grid}><div style={panel}>{txt(profile.profilePhoto)?<img src={txt(profile.profilePhoto)} alt="Profile" style={photoStyle}/>:null}<div style={eyebrow}>Member Profile</div><h2 style={h2}>{txt(profile.name,"VaultForge Member")}</h2><p style={sub}>{txt(profile.company,"Company not listed")}</p><p style={muted}>Type: {txt(profile.memberType,"Investor")} • City: {txt(profile.primaryCity,"Not listed")} • County: {txt(profile.primaryCounty,"Not listed")}</p><p style={muted}>Strategies: {list(profile.strategies).join(", ")||"Not listed"}</p><p style={muted}>Can provide: {list(profile.canProvide).join(", ")||"Not listed"}</p><div style={{...row,marginTop:18}}><Link href="/messages" style={goldBtn}>Message Member</Link><Link href="/profile" style={btn}>Edit Profile</Link></div></div></div></Section>
  </div></main>
}
