...
export default function MessageCommandPage() {
  ...
  const laneTotals = useMemo(() => {
    const next: Record<string, LaneTotals> = {};
    ...
    for (const convo of conversations) {
      ...
      if (!next[folder]) next[folder] = { conversations: 0, messages: 0, unread: 0 };
    ...
    try {
      ...
      if (action === "archive" || action === "delete") {
        ...
        setConversations((current) =>
          current.map((item) => (item.thread_key === convo.thread_key ? { ...item, is_saved: true } : item))
        );
        setStatus("Conversation saved.");
        await load();
      } else if (action === "unsave") {
        setConversations((current) =>
          current.map((item) => (item.thread_key === convo.thread_key ? { ...item, is_saved: false } : item))
        );
        setStatus("Removed from Saved Bucket.");
        await load();
      } else if (action === "read") {
        setConversations((current) =>
          current.map((item) => (item.thread_key === convo.thread_key ? { ...item, unread_count: 0, is_read: true } : item))
        );
        ...
        {!openLane ? (
          <>
            ...
            <section className="vf-card-grid" style={cardGrid}>
              {LANES.map((lane) => {
                const totals = laneTotals[lane.key] || { conversations: 0, messages: 0, unread: 0 };

                return (
                  <button key={lane.key} type="button" onClick={() => openRoute(lane.key)} style={routeCard}>
              ...
              {visible.map((convo) => {
                ...
                return (
                  <article key={convo.thread_key} style={conversation}>
                    ...
                    {!isCollapsed ? <p style={preview}>{convo.latest_message || "No preview available."}</p> : null}
                    ...
                    <div className="vf-actions" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 18 }}>
                      ...
                      <button
                        type="button"
                        onClick={() => setCollapsed((old) => ({ ...old, [convo.thread_key]: !isCollapsed }))}
                        style={ghost}
                      >
...
const wrap: React.CSSProperties = { width: "min(1260px,100%)", margin: "0 auto" };
...
const hero: React.CSSProperties = { border: "1px solid rgba(232,196,107,.22)", borderRadius: 30, padding: 28, marginBottom: 24, background: "linear-gradient(145deg,rgba(255,255,255,.07),rgba(255,255,255,.025))", boxShadow: "0 30px 90px rgba(0,0,0,.34)" };
const eyebrow: React.CSSProperties = { color: "#e8c46b", letterSpacing: ".18em", textTransform: "uppercase", fontWeight: 950, fontSize: 12 };
const heroTitle: React.CSSProperties = { fontSize: "clamp(52px,10vw,98px)", lineHeight: .86, letterSpacing: "-.075em", margin: "12px 0 20px" };
const lead: React.CSSProperties = { color: "#cbd5e1", fontSize: 20, lineHeight: 1.55, maxWidth: 920 };
...
const sectionTitle: React.CSSProperties = { fontSize: "clamp(36px,6vw,58px)", lineHeight: 1, letterSpacing: "-.055em", margin: "8px 0 8px" };
const cardGrid: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 16 };
const routeCard: React.CSSProperties = { position: "relative", minHeight: 235, border: "1px solid rgba(255,255,255,.12)", borderRadius: 30, padding: 24, background: "linear-gradient(145deg,rgba(255,255,255,.055),rgba(255,255,255,.025))", color: "white", textAlign: "left", cursor: "pointer", overflow: "hidden" };
const routeLabel: React.CSSProperties = { color: "#38bdf8", letterSpacing: ".18em", textTransform: "uppercase", fontWeight: 950, fontSize: 13 };
const routeNumber: React.CSSProperties = { position: "absolute", top: 24, right: 24, color: "#f8e7b0", fontSize: 64, fontWeight: 1000, lineHeight: 1 };
const routeTitle: React.CSSProperties = { fontSize: 38, lineHeight: 1, letterSpacing: "-.045em", margin: "56px 0 12px" };
const smallPill: React.CSSProperties = { borderRadius: 999, border: "1px solid rgba(255,255,255,.12)", padding: "8px 10px", color: "#dbeafe", fontSize: 12 };
const unreadPill: React.CSSProperties = { ...smallPill, color: "#fecaca", border: "1px solid rgba(248,113,113,.32)", background: "rgba(248,113,113,.10)", fontWeight: 950 };
const openHint: React.CSSProperties = { marginTop: 18, fontWeight: 950, color: "#f8e7b0" };
const laneOpenPanel: React.CSSProperties = { display: "flex", justifyContent: "space-between", gap: 18, flexWrap: "wrap", border: "1px solid rgba(232,196,107,.22)", borderRadius: 30, padding: 24, marginBottom: 18, background: "linear-gradient(145deg,rgba(232,196,107,.10),rgba(255,255,255,.035))" };
const laneStats: React.CSSProperties = { display: "flex", gap: 12, flexWrap: "wrap" };
const statBox: React.CSSProperties = { border: "1px solid rgba(255,255,255,.12)", borderRadius: 22, padding: 16, minWidth: 145, background: "rgba(0,0,0,.16)" };
const statNumber: React.CSSProperties = { display: "block", fontSize: 34, lineHeight: 1, color: "#f8e7b0" };
const searchPanel: React.CSSProperties = { border: "1px solid rgba(255,255,255,.10)", borderRadius: 24, padding: 16, marginBottom: 18, background: "rgba(255,255,255,.025)" };
const input: React.CSSProperties = { width: "100%", boxSizing: "border-box", borderRadius: 18, border: "1px solid rgba(255,255,255,.14)", background: "#081224", color: "white", padding: 16, fontSize: 16, outline: "none" };
...
const emptyPanel: React.CSSProperties = { border: "1px solid rgba(232,196,107,.18)", borderRadius: 28, padding: 22, marginTop: 18, background: "rgba(255,255,255,.035)" };
