...
export default function Deals() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('deals').insert([{
      ...
      status: analysis.status === 'REJECT'? 'REJECTED' : 'active'
    }])
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh', padding: '16px' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div className="grid-header">DEAL ROOM // VAULTFORGE INTEL // LIVE SUBMISSIONS</div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '16px' }}>

          <div>
            <div className="grid-header" style={{ marginBottom: '1px' }}>SUBMIT DEAL // INTEL ANALYSIS ACTIVE</div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1px', background: '#27272a' }}>
              ...
              <div className="grid-panel" style={{ padding: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                  ...
                  <div>
                    <div className="field-label">Rehab</div>
              ...
              <div className="grid-panel" style={{ padding: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '8px' }}>
                  <div>
                    ...
                    <input type="number" placeholder="0" value={form.beds} onChange={e => setForm({...form, beds: e.target.value})}
                      style={{ width: '100%', background: '#0a0a0b', border: '1px solid #27272a', padding: '8px', color: '#fafafa' }} />
                  </div>
                  <div>
                    <div className="field-label">Baths</div>
                    <input type="number" placeholder="0" value={form.baths} onChange={e => setForm({...form, baths: e.target.value})}
                      style={{ width: '100%', background: '#0a0a0b', border: '1px solid #27272a', padding: '8px', color: '#fafafa' }} />
                  </div>
                  <div>
              ...
              <div className="grid-panel" style={{ padding: '16px' }}>
                <div className="field-label">Who Is This Deal For</div>
              ...
              <div className="grid-panel" style={{ padding: '16px' }}>
                <div className="field-label">Deal Notes</div>
                <textarea placeholder="REPAIR ESTIMATE, RENTAL INCOME, NOTES" rows={2} value={form.description}
          ...
          <div>
            <div className="grid-header" style={{ marginBottom: '1px' }}>LIVE DEAL FEED // {deals.length} ACTIVE</div>
            <div style={{ display: 'grid', gap: '1px', background: '#27272a' }}>
              {deals.map(deal => (
                <div key={deal.id} className="grid-panel" style={{ padding: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
                    <div>
                      <div style={{ fontSize: '12px', fontWeight: '700', color: '#fafafa' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <div style={{
                      fontSize: '9px',
                      padding: '2px 6px',
                      background: deal.dqi_score >= 70? '#4ade80' : deal.dqi_score >= 50? '#fbbf24' : '#f87171',
                      color: '#000',
                      fontWeight: '900'
                    }}>
                      DQI {deal.dqi_score}
                    ...
                    {deal.photo_urls && deal.photo_urls.length > 0 && (
                      <div style={{ fontSize: '9px', color: '#facc15' }}>
                        {deal.photo_urls.length} PHOTOS
                      </div>
                    )}
                  </div>
                </div>
              ...
              {deals.length === 0 && (
                <div className="grid-panel" style={{ padding: '24px', textAlign: 'center', color: '#71717a', fontSize: '11px' }}>
                  NO DEALS IN SYSTEM // SUBMIT FIRST INTEL
                </div>
              )}
            </div>
          </div>
