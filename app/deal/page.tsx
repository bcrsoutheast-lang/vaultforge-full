...
function Card({ room }: { room: CanonicalRoom }) {
  ...
  return (
    <article className="vf-room-card">
      ...
      <Link href={`/deal-rooms/${encodeURIComponent(room.id)}`} className="vf-photo">
      ...
      <div className="vf-body">
        ...
        <div className="vf-pills">
          ...
          {room.strategy ? <span>{room.strategy}</span> : null}
        </div>

        <p className="vf-summary">{compact(room.ai_summary || room.summary || room.notes || "Open room for deal numbers, AI analysis, matched profiles, alerts, and routing context.")}</p>

        <div className="vf-actions">
          <Link href={`/deal-rooms/${encodeURIComponent(room.id)}`}>Open Room</Link>
          <Link href={`/message-command/${encodeURIComponent("deal:" + room.id)}`}>Thread</Link>
        </div>
      </div>
    </article>
  );
  ...
  return (
    <main className="vf-page">
      <style>{`
        ...
        .vf-hero,.vf-panel{border:1px solid rgba(245,197,91,.24);background:linear-gradient(145deg,rgba(16,24,36,.94),rgba(2,6,23,.98));border-radius:24px;padding:20px;box-shadow:0 24px 70px rgba(0,0,0,.28)}
        ...
        .vf-nav a,.vf-actions a{color:#f8fafc;text-decoration:none;border:1px solid rgba(245,197,91,.25);background:rgba(245,197,91,.07);border-radius:999px;padding:10px 13px;font-weight:900;font-size:13px}
        .vf-nav a.primary,.vf-actions a:first-child{background:linear-gradient(135deg,#fde68a,#e8c46b);color:#111827;border:0}
        .vf-grid{display:grid;gap:14px}
        .vf-room-card{border:1px solid rgba(245,197,91,.22);background:radial-gradient(circle at top right,rgba(245,197,91,.09),transparent 28%),linear-gradient(145deg,rgba(12,16,25,.96),rgba(2,6,23,.99));border-radius:24px;padding:14px;display:grid;grid-template-columns:130px minmax(0,1fr);gap:14px}
        .vf-line{grid-column:1/-1;height:4px;border-radius:999px;background:linear-gradient(90deg,#f5c55b,transparent)}
        .vf-photo{height:130px;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,.12);background:radial-gradient(circle at top left,rgba(245,197,91,.18),transparent 30%),linear-gradient(135deg,#111827,#020617)}
        ...
        .vf-score strong{display:block;color:#f5c55b;font-size:30px;line-height:1}
        ...
        .vf-pills span{border:1px solid rgba(255,255,255,.13);background:rgba(255,255,255,.045);color:#cbd5e1;border-radius:999px;padding:6px 9px;font-size:11px;font-weight:850}
        ...
        @media(max-width:700px){.vf-room-card{grid-template-columns:1fr}.vf-photo{height:160px}.vf-card-top h2{font-size:24px}}
      `}</style>
      ...
      <div className="vf-wrap">
        <section className="vf-hero">
          ...
          <h1>Deal Rooms</h1>
          <p>One clean lane for deals, projects, and opportunities. Projects are no longer a separate member-facing system. Full numbers, AI, profiles, alerts, and routing live inside each room.</p>
          <div className="vf-nav">
            <Link href="/dashboard">Command</Link>
            <Link href="/deal-rooms" className="primary">Deal Rooms</Link>
            <Link href="/pain-rooms">Pain Rooms</Link>
            <Link href="/submit">Create Deal</Link>
          </div>
        </section>

        <section className="vf-panel">
          <div className="vf-kicker">Active Opportunity Cards</div>
          {!rooms.length ? <p style={{ color: "#cbd5e1" }}>No deal records resolved yet. This page is live; the next fix would be table/column mapping if Supabase records exist.</p> : null}
