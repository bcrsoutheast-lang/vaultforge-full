...
const shell: React.CSSProperties = { minHeight: "100vh", background: "radial-gradient(circle at top left, rgba(232,196,107,.16), transparent 30%), linear-gradient(180deg, #030509 0%, #071326 55%, #030509 100%)", color: "white", padding: "26px 18px 90px", fontFamily: "Arial, sans-serif" };
...
const navLink: React.CSSProperties = { color: "#06100a", background: "#f5d978", textDecoration: "none", borderRadius: 999, padding: "12px 16px", fontWeight: 900, border: "none" };
const ghost: React.CSSProperties = { color: "white", textDecoration: "none", border: "1px solid rgba(255,255,255,.18)", borderRadius: 999, padding: "12px 16px", fontWeight: 900, background: "rgba(255,255,255,.04)" };
...
function Field({ label, value }: { label: string; value: any }) {
  ...
  return <div style={section}><div style={eyebrow}>{label}</div><p style={{ ...muted, fontSize: 20, margin: 0 }}>{Array.isArray(value) ? value.join(", ") : String(value)}</p></div>;
}
...
export default function DealRoomPage() {
  ...
  async function loadDeal() {
    ...
    try {
      ...
      if (!res.ok) setStatus(data?.error || data?.details || "Could not load deal.");
      else setDeal(data?.deal || null);
    } catch { setStatus("Could not load deal. Refresh and try again."); }
    setLoading(false);
  }
  async function sendMessage() {
    setMessageStatus("");
    if (!message.trim()) { setMessageStatus("Write a message first."); return; }
    try {
      const res = await fetch("/api/messages/send", { method: "POST", headers: headers(), body: JSON.stringify({ deal_id: id, subject: `Inquiry on ${deal?.title || "VaultForge deal"}`, body: message.trim() }) });
      const data = await res.json();
      ...
      <div style={wrap}>
        ...
        {deal && <>
          ...
          <section style={section}>
            ...
            {photos.length === 0 ? <p style={muted}>No photos uploaded for this deal.</p> : <div style={grid}>{photos.map((src, i) => <img key={`${src}-${i}`} src={src} alt={`Deal photo ${i + 1}`} style={{ ...image, height: i === 0 ? 360 : 240 }} />)}</div>}
          </section>

          <section style={section}>
            <div style={eyebrow}>MESSAGE DEAL OWNER</div>
            <h2 style={{ fontSize: 34, margin: "0 0 10px" }}>Ask about this opportunity.</h2>
            <p style={muted}>Send a deal-tied message. It is stored with this opportunity for follow-up.</p>
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="I'm interested in this deal. Can you send more details or next steps?" style={{ ...input, minHeight: 130, resize: "vertical" }} />
            <button type="button" onClick={sendMessage} style={{ ...navLink, marginTop: 12 }}>Message Owner</button>
            {messageStatus && <p style={{ color: messageStatus.toLowerCase().includes("sent") ? "#9df3bf" : "#ffd0d0", fontWeight: 900 }}>{messageStatus}</p>}
          </section>
