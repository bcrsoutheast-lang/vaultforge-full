...
function smartBrief(deal: Deal | null) {
  ...
  const market = first(
    [from(deal, "city"), from(deal, "state", "market")].filter(Boolean).join(", "),
    from(deal, "market", "location", "address")
  );
  ...
  return [
    `${lead}.`,
    economics.length ? `Economics: ${economics.join(" / ")}. ${spreadText(deal)}`.trim() : "",
    execution.length ? `Execution read: ${execution.join(" / ")}.` : "",
    `Best-fit route: ${bestFit(deal)}.`,
  ]
    .filter(Boolean)
    .join(" ");
}
...
function Info({ title, value }: { title: string; value: unknown }) {
  return (
    <div style={card}>
      ...
      <div style={{ fontSize: 28, fontWeight: 900 }}>{display(value)}</div>
    </div>
  );
}
...
export default function DealDetailPage() {
  ...
  const detailTitle = first(
    from(deal, "title", "deal_title", "project_title"),
    from(deal, "address", "property_address"),
    "Deal Detail"
  );
      ...
      <div style={wrap}>
        ...
        {deal ? (
          <>
            ...
            <section style={card}>
              <div style={label}>Bloomberg AI Brief</div>
              <div style={{ fontSize: 22, lineHeight: 1.45 }}>{smartBrief(deal)}</div>
            </section>

            <section style={card}>
              <div style={label}>Routing Needs</div>
              <div style={{ fontSize: 22 }}>{display(from(deal, "routing_needs", "deal_needs", "needs"))}</div>
            </section>

            <section style={card}>
              <div style={label}>Distress / Pain Signals</div>
              <div style={{ fontSize: 22 }}>{display(from(deal, "distress_signals", "seller_pressure"))}</div>
            </section>

            <section style={card}>
              <div style={label}>Seller / Situation</div>
              <div style={{ fontSize: 22 }}>
                {display(from(deal, "seller_situation", "private_notes", "access_notes", "distress_signals"))}
              </div>
            </section>
