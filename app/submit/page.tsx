  ...
  return (
    <main style={shellStyle}>
      ...
      <section style={cardStyle}>
        <label style={{ display: "block", fontWeight: 800, marginBottom: 8 }}>Title *</label>
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Example: ATL Flip" style={inputStyle} />

        <label style={{ display: "block", fontWeight: 800, marginBottom: 8 }}>State *</label>
        <select value={state} onChange={(e) => setState(e.target.value)} style={inputStyle}>{STATES.map((item) => <option key={item}>{item}</option>)}</select>

        <label style={{ display: "block", fontWeight: 800, marginBottom: 8 }}>Property Type *</label>
        <select value={propertyType} onChange={(e) => setPropertyType(e.target.value)} style={inputStyle}>{PROPERTY_TYPES.map((item) => <option key={item}>{item}</option>)}</select>

        <label style={{ display: "block", fontWeight: 800, marginBottom: 8 }}>Strategy / Bucket</label>
        <select value={strategy} onChange={(e) => setStrategy(e.target.value)} style={inputStyle}>{STRATEGIES.map((item) => <option key={item}>{item}</option>)}</select>

        <label style={{ display: "block", fontWeight: 800, marginBottom: 8 }}>Price</label>
        <input value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Example: 250000" style={inputStyle} />
      ...
      {aiSummary && (
        <section style={cardStyle}>
          <p style={{ color: "#9df3bf", letterSpacing: 4, fontWeight: 800 }}>AI DEAL ANALYSIS</p>
          <p style={{ color: "rgba(255,255,255,.78)", fontSize: 20, lineHeight: 1.5 }}>{aiSummary}</p>
        </section>
      )}
    </main>
  );
