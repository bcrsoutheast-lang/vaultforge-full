...
export default function CommandPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setRows(unique);
      setStatus(unique.length ? "" : "No introductions records found yet.");
    } catch (error: any) {
  ...
  return (
    <main style={page}>
      ...
      <div style={wrap}>
        <VaultForgeMemberNav title="Introductions" subtitle="Controlled introductions with dashboard access, exact intro cards, and message routing." active="introductions" />

        <section style={card}>
          <p style={eyebrow}>VaultForge Introductions</p>
          <h1 style={{ fontSize: "clamp(50px,10vw,92px)", lineHeight: 0.88, margin: "10px 0 18px", letterSpacing: "-.06em" }}>
            Introductions.
          </h1>
          <p style={{ ...muted, fontSize: 18 }}>Controlled introductions with dashboard access, exact intro cards, and message routing.</p>
        ...
        <section className="vf-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(330px,1fr))", gap: 16 }}>
          {filtered.map((row, index) => (
            <RecordCard key={`${signalIdOf(row)}-${itemIdOf(row)}-${row.id || index}`} row={row} email={email} mode="introductions" />
          ))}
        </section>
      </div>
    </main>
