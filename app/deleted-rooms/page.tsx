...
const TARGET: RoomState = "deleted";
const LABEL = "Deleted";
...
function text(value: unknown, fallback = "") { const clean = String(value || "").trim(); return clean || fallback; }
function roomId(room: RoomRecord | null | undefined) { return text(room?.id || room?.roomId || room?.dealId || room?.painId, ""); }
function readArray(key: string): RoomRecord[] { const parsed = parseJson<unknown>(window.localStorage.getItem(key), []); return Array.isArray(parsed) ? parsed as RoomRecord[] : []; }
...
function readRooms(kind: RoomKind) {
  ...
  for (let i = 0; i < window.localStorage.length; i += 1) {
    ...
    const isDeal = kind === "deal" && (key.startsWith("vaultforge_clean_deal_room_") || key.startsWith("vaultforge_deal_room_"));
    const isPain = kind === "pain" && (key.startsWith("vaultforge_clean_pain_room_") || key.startsWith("vaultforge_pain_room_"));
...
function deleteForever(room: RoomRecord, kind: RoomKind) {
  const ok = window.confirm(`Delete "${text(room.title || room.name, "room")}" forever from local storage?`);
  if (!ok) return;
...
function href(kind: RoomKind, room: RoomRecord) {
  return kind === "deal" ? `/deal-rooms/${encodeURIComponent(roomId(room))}` : `/pain-rooms/${encodeURIComponent(roomId(room))}`;
}

function location(room: RoomRecord) {
  return [text(room.city), text(room.county), text(room.state)].filter(Boolean).join(", ") || "Market not listed";
}

export default function RoomFolderPage() {
  const [tick, setTick] = useState(0);

  function load() { setTick((value) => value + 1); }
    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <section style={card}>
          ...
          <p style={sub}>Deal and Pain rooms moved out of Active stay here until restored or deleted forever.</p>
        </section>

        <RoomSection title="Deal Rooms" kind="deal" rooms={deals} onMove={syncState} onDeleteForever={deleteForever} />
        <RoomSection title="Pain Rooms" kind="pain" rooms={pains} onMove={syncState} onDeleteForever={deleteForever} />
      </div>
    </main>
  );
}
...
function RoomSection({
  ...
  return (
    <section style={card}>
      <div style={eyebrow}>{title} ({rooms.length})</div>
      {!rooms.length ? <p style={sub}>No {title.toLowerCase()} in this folder.</p> : null}
      <div style={grid}>
        {rooms.map((room) => (
          <article key={`${kind}-${roomId(room)}`} style={roomCard}>
            <div style={miniEyebrow}>{kind === "deal" ? "Deal Room" : "Pain Room"}</div>
            <h2 style={roomTitle}>{text(room.title || room.name, kind === "deal" ? "Untitled Deal" : "Untitled Pain")}</h2>
            ...
            <div style={actionRow}>
              ...
              {TARGET === "deleted" ? <button type="button" onClick={() => onDeleteForever(room, kind)} style={dangerBtn}>Delete Forever</button> : null}
            </div>
          </article>
        ))}
      </div>
...
const grid: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: 16 };
...
const muted: React.CSSProperties = { color: "#aeb7c7", margin: "0 0 14px" };
