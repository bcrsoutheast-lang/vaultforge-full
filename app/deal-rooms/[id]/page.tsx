...
export default function DealRoomPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    setRoom(found);
    if (found) markRead("deal", found);
  }, [id]);

  const members = useMemo(() => getDirectory(), [room?.id]);
  const intelligence = room ? dealIntelligence(room) : null;
  const matches = room ? members.map((member) => scoreMemberForRoom(member, room, "deal")).sort((a, b) => b.score - a.score).slice(0, 5) : [];

  if (!room || !intelligence) {
    return <main style={page}><div style={wrap}><Nav active="deals" /><section style={hero}><h1 style={h1}>Room not found.</h1><p style={sub}>Go back to Deal Rooms and open a current room.</p></section></div></main>;
  }
  ...
  return <main style={page}><div style={wrap}><Nav active="deals" />

    <section style={intelligence.urgency >= 80 ? redHero : hero}>
      {img ? <img src={img} alt={titleFor(room, "deal")} style={photoStyle} /> : null}
      <div style={eyebrow}>Deal Room • {roomState(room)}</div>
